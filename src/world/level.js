...
export function buildLevel(mats, { lamps = true, mapId = 'urban' } = {}) {
  ...
  wallRun(M.concrete, 0, -6.5, 0, 15, bunkerH, 1.0,
    [slit(-5.3, 1.1), slit(-0.4, 1.9), slit(4.9, 1.5, 1.2)], 3);
  wallRun(M.concrete, 0, 6.5, 0, 15, bunkerH, 1.0,
    [slit(-4.6, 1.8), { u: 0.6, w: 2.6, y0: 0, y1: 2.5 }, slit(5.2, 1.15)], 3);
  wallRun(M.concrete, -7.0, 0, Math.PI / 2, 13, bunkerH, 1.0, [slit(-3.9, 1.35), slit(3.4, 1.75)], 3);
  ...
  for (const sy of [0.55, 1.15, 1.75]) {
    box(0.55, 0.06, 3.6, M.metal, -6.1, sy, -1.2, 0, 1.0);
  }
  for (const sz of [-2.9, 0.5]) {
  ...
  for (let i = 0; i < 5; i++) {
    const t = (i - 2) * 0.99;
    const h0 = sandbagAt(-3 + t, bunkerRoof, -6.6, rr(-0.12, 0.12), rr(0.94, 1.06), rr(0.84, 0.93));
    // 上段は半ピッチずらして目地を互い違いにする
    if (i > 0 && i < 4) {
      sandbagAt(-3 + t + 0.5, bunkerRoof + h0 - 0.055, -6.6, rr(-0.18, 0.18), rr(0.94, 1.06), rr(0.92, 1.0));
    }
  }
  ...
  rubble(2.0, -4.2, 1.6, 6, [M.concrete, M.brick]);
  ladder(7.9, 0, 3.4, 3.9, Math.PI / 2);

  // 掩体を囲む車止め。胸の高さの遮蔽を中央に置いて、距離を詰める側にも寄る場所を作る
  jerseyLine(-2.5, -11.0, 0, 4);
  jerseyLine(4.0, 11.6, 0, 3);
  ...
  // 掩体の屋上(3.9)から東のコンテナ天端(5.22)へ渡す。屋根から屋根の線を1本通す
  containerBody(13.5, 0, 0, M.corr, 2);
  stepBlocks(13.5, -7.22, 0, 6, M.concrete, 0.78, 1.0, 1.7);
  plank(1.6, 3.2, 1.46, M.metal, 9.0, 3.9, 0, Math.PI / 2);
  ...
  /* ----------------------------------------------- 柱・ドラム缶・配管 */
  [[-6, 8, 0], [-7.2, 9.1, 1], [14, 18, 2], [15.1, 17.2, 0], [-19, -2, 1],
