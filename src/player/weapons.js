...
/* ライフルの桜。**2026-08-11に作り直した。**
   ...
   **銃の0.7%の物を貼っていただけ**なので輪郭が1mmも変わっていなかった。
   ...
   今回は**輪郭から変える。**
     ・銃口を花弁の形の制退器にする（5枚が開いて、先端が花になる）
     ・花を半径35mmまで大きくする（前の4倍）
     ・銃床から花吹雪を流す
   **銃口の先は照準線の外**なので、そこは何を付けても構わない
   （赤ドットの円錐は前へ行くほど太くなるが、照星より前は円錐の外）。
   窓が狭まっていないかは tools/check-scope.mjs の[4]が測る */
function sakuraDeco(g) {
  const rear = g.userData.rear;

  /* ---- 銃口の花弁。**ここが一番効く。**
     5枚の花弁を放射状に開く。1枚が長さ48mm・幅30mmで、
     **輪郭が「筒」から「花」に変わる。**
     潰した球を回して並べる（押し出した多角形より軽く、丸みも出る）*/
  for (let i = 0; i < 5; i++) {
    const a2 = (i / 5) * Math.PI * 2;
    const r = 0.030;
    g.add(partS(sphG(0.024, 10, 7), MATS.candyPink,
      Math.cos(a2) * r, R_BORE + Math.sin(a2) * r, -0.676,
      1.0, 1.0, 0.30, 0, 0, a2));
  }
  // 花弁の芯。銃口を囲む輪。花の中心に見える
  g.add(part(torG(0.020, 0.0060, 6, 16), MATS.ivory, 0, R_BORE, -0.664));
  g.add(part(sphG(0.011, 10, 8), MATS.charm, 0, R_BORE, -0.658));

  /* ---- 大きい花。**半径35mmまで使う**（前は8.5mm）。
     先台の側面と天面に3輪。天面はレール(0.052)より下で止める */
  const flower = (x, y, z, r, rot) => {
    for (let i = 0; i < 5; i++) {
      const a2 = (i / 5) * Math.PI * 2;
      g.add(partS(sphG(r, 8, 6), MATS.candyPink,
        x + Math.cos(a2) * r * 1.05, y + Math.sin(a2) * r * 1.05 * (rot ? 0 : 1),
        z + (rot ? Math.sin(a2) * r * 1.05 : 0),
        1, rot ? 1 : 0.30, rot ? 0.30 : 1));
    }
    g.add(partS(sphG(r * 0.42, 8, 6), MATS.charm, x, y, z, 1, 0.5, 1));
  };
  flower(0.0300, R_BORE - 0.004, -0.330, 0.030, true);
  flower(-0.0300, R_BORE + 0.002, -0.450, 0.026, true);
  flower(0, R_RAIL - 0.012, -0.250, 0.028, false);
...
function buildGlove(view = {}) {
  ...
  // 指の面。前を平らに落とすと「握った拳」に見える（球のままだと風船）
  g.add(part(cboxG(0.086, 0.076, 0.026), MATS.mitt, 0, 0.000, -0.082));
  /* 親指の膨らみ。**内側へ1つ。** これが無いと左右対称の玉になって、
     グローブではなくボールに見える */
  g.add(partS(sphG(0.026, 10, 8), MATS.mitt, -0.044, -0.014, -0.048, 1.0, 0.9, 1.3, 0, 0.3, 0));
  /* 甲の縫い目。**革が縫い合わさっている線を1本。**
     無地の塊だとゴムの塊に見える */
  g.add(part(boxG(0.004, 0.052, 0.086), MATS.strap, 0, 0.030, -0.044));
  // 当て金。**丸い塊の上に薄く3枚だけ。**主役は塊なので、金は添え物にする
  for (let i = 0; i < 3; i++) {
    g.add(part(cboxG(0.018, 0.008, 0.022), MATS.brass, -0.022 + i * 0.022, 0.034, -0.070));
  }
  ...
  g.add(part(cylG(0.037, 0.037, 0.008, 14), MATS.strap, 0, -0.004, 0.046, Math.PI / 2));
  // 留めの金具
  g.add(part(cboxG(0.016, 0.012, 0.028), MATS.brass, 0.032, -0.004, 0.030));
  ...
  g.add(part(cboxG(0.072, 0.064, 0.022), MATS.mitt, -0.082, -0.038, -0.048));
  for (let i = 0; i < 3; i++) {
