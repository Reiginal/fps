...
function buildGlove(view = {}) {
  ...
  handL.position.set(-0.078, -0.034, 0.022);
  handL.rotation.set(-0.24, 0.18, -0.20);
  g.add(handL);
  g.userData.handL = handL;
  g.add(partS(sphG(0.046, 12, 9), MATS.mitt, -0.082, -0.038, -0.010, 1.0, 0.94, 1.15));
  // 左も板をやめて丸で終わらせる（右と同じ理由）
  g.add(partS(sphG(0.042, 12, 9), MATS.mitt, -0.082, -0.040, -0.040, 1.0, 0.92, 0.78));
  g.add(part(torG(0.041, 0.0036, 6, 20), MATS.ivory, -0.082, -0.040, -0.034, 0, 0, 0));
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.030, 0.028, 0.016, 12), MATS.mitt, -0.082, -0.040, 0.026 + i * 0.018, Math.PI / 2));
  }
