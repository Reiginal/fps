...
function buildGlove(view = {}) {
  ...
  /* ---- 左のミット。**meleeRigは右手しか作らない**ので、こちらで足す。
     右より一回り小さく・奥に置く（構えた時に左が前へ出過ぎない）*/
  const handL = buildHand(-1, {
    gripR: 0.024, wrap: 0.92, trigger: false,
    armDir: grip.armDirL || [-0.42, -0.62, 0.86],
    armLen: 0.58,
  });
  handL.position.set(-0.144, 0.112, -0.050);
  handL.rotation.set(-0.24, 0.18, -0.20);
  g.add(handL);
  g.userData.handL = handL;
  /* **左は画面へ入れる。**
     2026-08-12に「パンチグローブは普通は両手じゃない？」と言われた所。
     コメントには「両拳を出す」と書いてあったのに、
     **左は一度も画面に入っていなかった。**

     測って分かった形（画面320×180・画角55度に投げて調べた）:
       右ミット … 画面(251, 156)。下の縁に半分掛かって見えている
       左ミット … 画面(250, 200)。**縦が180を越えていて画面の下**
     右のすぐ真下、しかも枠の外。だから「片手だけ」に見えていた。

     上へ110mm・左へ66mm動かして画面(172, 170)へ入れる。
     右とは横に80画素離れるので、2つの拳として読める
     （ミット1個の幅が44画素なので、重ならない）*/
  g.add(partS(sphG(0.046, 12, 9), MATS.mitt, -0.148, 0.108, -0.090, 1.0, 0.94, 1.15));
  // 左も板をやめて丸で終わらせる（右と同じ理由）
  g.add(partS(sphG(0.042, 12, 9), MATS.mitt, -0.148, 0.106, -0.120, 1.0, 0.92, 0.78));
  g.add(part(torG(0.041, 0.0036, 6, 20), MATS.ivory, -0.148, 0.106, -0.114, 0, 0, 0));
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.030, 0.028, 0.016, 12), MATS.mitt, -0.148, 0.106, -0.054 + i * 0.018, Math.PI / 2));
  }
