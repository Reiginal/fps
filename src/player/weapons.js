...
function skeletonDeco(g) {
  ...
  /* ---- 中に見える発条（ばね）。**輪を並べて巻き線に見せる。**
     本物の螺旋を作ると面が増えるだけで、この大きさでは輪と区別が付かない */
  for (let i = 0; i < 7; i++) {
    g.add(part(torG(0.0055, 0.0014, 4, 8), MATS.steel,
      0, P_BORE + 0.010, -0.104 + i * 0.016, Math.PI / 2));
  }
  /* ---- 中に見える真鍮の弾。**握把の中に3発。**
     透ける枠の中で唯一の暖色なので、これが「中が見える」の合図になる */
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.0042, 0.0042, 0.020, 8), MATS.brass,
      0, -0.056 - i * 0.016, 0.036 + i * 0.005, P_GRIP_TILT, 0, Math.PI / 2));
  }
  // 撃鉄の発条。銃尾に1つだけ見せる
  g.add(part(torG(0.0060, 0.0016, 4, 10), MATS.steel, 0, P_BORE + 0.008, 0.030, Math.PI / 2));
