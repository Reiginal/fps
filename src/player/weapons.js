...
/* 狙撃銃の星（アストロ）。**望遠鏡に見立てる。狙撃銃だけが持てる比喩。**
 *
 * アイス（白と氷）・ヴェノム（黄緑と鱗）に対して、こちらは**紺紫と光。**
 * 光らせる方向はサイバー（青緑）とヴェノム（黄緑）が使っているので、
 * **淡い白**にして分ける（星は色を持たない方が星に見える）。
 *
 * 望遠照準の視界を塞がないのはアイスと同じ制約で、
 * 機関部の上（レール0.042〜光軸0.072の間）は空けたまま。
 * 光る環は**対物側の前**へ置く（そこは視線の円錐の外）。
 */
function astroDeco(g) {
  const rear = g.userData.rear;

  /* ---- 星の粒。**先台と機関部の側面に散らす。**
     等間隔にしないのが要点で、並べると計器のランプに見える。
     数は12個まで。増やすと暗い場所で銃が提灯になる（ドラゴンの目と同じ理由） */
  const stars = [
    [0.0268, 0.020, -0.090], [0.0268, -0.004, -0.180], [0.0268, 0.014, -0.310],
    [0.0268, -0.010, -0.395], [-0.0268, 0.016, -0.060], [-0.0268, -0.006, -0.150],
    [-0.0268, 0.018, -0.270], [-0.0268, -0.002, -0.360], [0, 0.038, -0.230],
    [0, 0.038, -0.330], [0.0268, 0.008, -0.430], [-0.0268, 0.010, -0.420],
  ];
  for (const [x, y, z] of stars) {
    g.add(part(sphG(0.0034, 6, 5), MATS.starLight, x, S_BORE + y, z));
  }

  /* ---- 光る環。**制退器の前。** 望遠鏡の対物レンズに見立てる。
     視線の円錐は前へ行くほど太くなるが、ここは銃口より前なので入らない */
  g.add(part(torG(0.0225, 0.0028, 5, 18), MATS.starLight, 0, S_BORE, -0.842));
  g.add(part(torG(0.0180, 0.0018, 5, 16), MATS.starLight, 0, S_BORE, -0.826));

  /* ---- 環を支える筒。**光る物だけだと浮くので、金物の台を置く。**
     レールの高さ(0.042)を越えないので照準には入らない */
  g.add(part(cylG(0.0245, 0.0245, 0.030, 16, true), MATS.chrome, 0, S_BORE, -0.826, Math.PI / 2));

  /* ---- 先台の帯。星を散らした面を区切ると、夜空の絵に見える */
  for (const z of [-0.200, -0.330]) {
    g.add(part(cboxG(0.056, 0.052, 0.008), MATS.chrome, 0, S_BORE - 0.004, z));
  }

  // 銃床。頬当てに星を2つ（rearの中へ。構えると一緒に消える）
  if (rear) {
...
/* 狙撃銃の竹（バンブー）。**ウエスタンの胡桃とは別の木。**
   あちらは磨いた焦茶の家具の木で、こちらは節のある若い竹。
   同じ「木」でも、節があるかどうかで別の物に見える。

   望遠照準の制約はアイス・星と同じ（レールより上へ出さない） */
function bambooDeco(g) {
  const rear = g.userData.rear;

  /* ---- 竹の節。**これが竹の全部。** 銃身に環を等間隔で並べる。
     節が無いと「緑に塗った棒」にしかならない */
  for (let i = 0; i < 6; i++) {
    const z = -0.430 - i * 0.070;
    g.add(part(cylG(0.0185, 0.0185, 0.012, 12), MATS.bamboo, 0, S_BORE, z, Math.PI / 2));
    // 節の際の細い線。1本入れると生きた竹に見える
    g.add(part(torG(0.0190, 0.0012, 4, 12), MATS.bamboo, 0, S_BORE, z + 0.006));
  }
  // 銃身を竹の筒で包む。節の間を埋める
  g.add(part(cylG(0.0170, 0.0170, 0.400, 12), MATS.bamboo, 0, S_BORE, -0.590, Math.PI / 2));

  /* ---- 先台の竹。**角張った台を丸い筒で覆う。**
     元の台は伏せ撃ちの箱なので、丸くすると印象が一番変わる */
  ...
  /* ---- 麻紐の巻き。**握る所と継ぎ目に。** 竹は割れるので縛って補強する物で、
     紐が無いと「竹を模した樹脂」に見える */
  for (const z of [-0.150, -0.470]) {
    for (let i = 0; i < 3; i++) {
      g.add(part(cylG(0.0195, 0.0195, 0.010, 10), MATS.strap, 0, S_BORE, z + i * 0.013, Math.PI / 2));
    }
  }
