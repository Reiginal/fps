...
function buildKusarigama(view = {}) {
  ...
  /* ---- 鎖。**柄尻から垂らす。**
     節を1つずつ置いて、下がりながら手前へ流れる曲線にする。
     14節で足りる。増やすと面が増えるだけで、垂れている形は変わらない。
     **輪を交互に90度倒す**のが鎖の見分けで、同じ向きに並べると縄になる */
  const CH = 14;
  const at = (t) => [
    -0.012 - Math.sin(t * 2.1) * 0.026,
    0.002 - t * 0.190 - t * t * 0.070,
    0.128 + t * 0.052,
  ];
  for (let i = 0; i < CH; i++) {
    const p = at(i / CH);
    g.add(part(torG(0.0072, 0.0022, 4, 8), MATS.phosphate,
      p[0], p[1], p[2], Math.PI / 2, i % 2 ? Math.PI / 2 : 0, 0));
  }
  // 柄尻の環。鎖を留めている所
  ...
  const tip = at(1);
  g.add(partS(sphG(1), MATS.phosphate, tip[0], tip[1] - 0.020, tip[2], 0.020, 0.026, 0.020));
  g.add(part(torG(0.0080, 0.0020, 4, 10), MATS.phosphate, tip[0], tip[1] - 0.002, tip[2], Math.PI / 2));
...
function buildGlove(view = {}) {
  ...
  g.add(partS(sphG(0.055, 14, 10), MATS.mitt, 0, 0.000, -0.036, 1.0, 0.94, 1.15));
  /* 拳の前。**2026-08-12に平らな板から丸い塊へ変えた。**
     ...
     潰した球をもう1つ重ねて、輪郭を最後まで丸のまま終わらせる */
  g.add(partS(sphG(0.050, 14, 10), MATS.mitt, 0, -0.004, -0.070, 1.0, 0.92, 0.78));
  /* 親指の膨らみ。**内側へ1つ。** これが無いと左右対称の玉になって、
     グローブではなくボールに見える */
  g.add(partS(sphG(0.028, 10, 8), MATS.mitt, -0.046, -0.014, -0.046, 1.0, 0.9, 1.3, 0, 0.3, 0));
  /* 縁取り。**ボクシンググローブは白い縁で読む。**
     前は甲に細長い板を1枚立てていて、塗りで白くしたら
     赤い玉に白い棒が刺さっているだけになった。
     実物で白いのは「指の側をぐるりと回る縁」なので、輪で回す */
  g.add(part(torG(0.049, 0.0042, 6, 22), MATS.ivory, 0, -0.002, -0.062, 0, 0, 0));
  /* 紐（レース）。手首の甲に3本。**金属を1つも使わない。**
     前は当て金3枚と留め金具を真鍮で付けていたが、
     ボクシンググローブに金物は無い。あれが「拳闘の道具」から遠ざけていた */
  for (let i = 0; i < 3; i++) {
    g.add(part(boxG(0.026, 0.004, 0.005), MATS.ivory, 0, 0.028, 0.006 + i * 0.013));
  }

  /* ---- 手首の帯。**太く巻く。** グローブは「着けている物」なので、
     手首が絞られていないと手が生えているように見える。
     一番後ろだけ布(strap)にして、革との継ぎ目を作る */
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.036, 0.034, 0.018, 14), MATS.mitt, 0, -0.004, 0.014 + i * 0.020, Math.PI / 2));
  }
  g.add(part(cylG(0.037, 0.037, 0.010, 14), MATS.strap, 0, -0.004, 0.048, Math.PI / 2));

  /* ---- 左のミット。**meleeRigは右手しか作らない**ので、こちらで足す。

     **群れ(offhand)に入れる。** 2026-08-12に
     「クリックでどっちも前に出るのおかしい、片側だけにして」と言われた所。
     振りは模型ごと動かす作りなので、群れに分けておかないと
     打っていない方の拳まで一緒に前へ出る（_animateがここを逆へ戻す）。

     **左右の置き方も作り直した。**「なんか左右のバランスおかしい」。
     前は右が画面の右下で近く、左が下の真ん中で遠く、
     しかも大きさが違った（右55mm・左46mm）ので、
     **同じ物が2つあるように見えていなかった。**
     大きさを揃えて、画面の左右へ同じだけ振り分ける（構えたボクサーの形）*/
  ...
  off.add(partS(sphG(0.055, 14, 10), MATS.mitt, -0.210, 0.276, -0.064, 1.0, 0.94, 1.15));
  off.add(partS(sphG(0.050, 14, 10), MATS.mitt, -0.210, 0.272, -0.098, 1.0, 0.92, 0.78));
  off.add(partS(sphG(0.028, 10, 8), MATS.mitt, -0.166, 0.262, -0.074, 1.0, 0.9, 1.3, 0, -0.3, 0));
  off.add(part(torG(0.049, 0.0042, 6, 22), MATS.ivory, -0.210, 0.274, -0.090, 0, 0, 0));
  for (let i = 0; i < 3; i++) {
    off.add(part(boxG(0.026, 0.004, 0.005), MATS.ivory, -0.210, 0.304, -0.022 + i * 0.013));
  }
  for (let i = 0; i < 3; i++) {
    off.add(part(cylG(0.036, 0.034, 0.018, 14), MATS.mitt, -0.210, 0.272, -0.014 + i * 0.020, Math.PI / 2));
  }
  off.add(part(cylG(0.037, 0.037, 0.010, 14), MATS.strap, -0.210, 0.272, 0.020, Math.PI / 2));
  /* **群れの中も結合する。** bakeStaticは直下の面しか見ないので、
     ここで呼ばないと左の拳だけ面が15枚のまま残る（実際32個まで増えた）*/
  bakeStatic(off);
  g.add(off);
...
export class WeaponSystem {
  ...
  _animate(dt, input, player) {
    ...
    const off = w.parts.offhand;
    if (off) {
      const k = 1 / (v.scale || 1);
      off.position.set(0, -swingH * k, -swingZ * k);
    }
