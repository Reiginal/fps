...
const INSPECT = {
  ...
  shake: 0, shakeHz: 0,
  /* 別の群れを回す速さ（rad/秒）。**鎖鎌の鎖だけが使う。**
     模型ごとは回せない（腕が入っている）が、
     武器の一部を別の群れに分けておけばそこは回せる */
  spin: 0,
};
...
const INSPECTS = {
  ...
  /* 鎖鎌 … **鎖を振り回す。** 2026-08-12に「ブキミルモーションでその鎖を
     ブンブン回すとかでもいいしね」と言われた所。

     模型ごと回すことはできない（腕が同じ群れに入っている）が、
     **鎖だけは別の群れ(userData.chain)に入れてある**ので、そこを回せる。
     柄尻を原点にして回すので、分銅が手の先で円を描く。

     spin … 1秒あたりの回転(rad)。7.5なら1.4秒で約1.7周
     本体は傾けない。鎖が回っているのが主役なので、持ち上げるだけにする */
  kusarigama: { tilt: 0.06, yaw: 0.30, pitch: -0.14, z: 0.030, h: 0.050, time: 1.6, spin: 7.5 },
...
export const SWINGS = {
  ...
  /* ---- 2026-08-12に足した2つ。
     鎖鎌は**引き寄せて切る**道具、チェーンソーは**押し当てる**道具で、
     どちらも今までの「振り抜く／突く」と通る道が違う */

  /* 鎖鎌・左: 引っ掛けて手前へ引く。**振り抜かない。**
     鎌は刃が内側を向いているので、前へ振ると背中で殴ることになる。
     前へ出してから手前へ戻す（zがマイナスからプラスへ動く）のが要点 */
  'kusarigama.light': {
    time: 0.40, wind: 0.24, speed: 2.6, fade: 0.54,
    back: { p: -0.18, y: 0.30, r: -0.34, z: -0.16, h: 0.04 },
    thru: { p: 0.36, y: -0.34, r: 0.42, z: 0.10, h: -0.06 },
    arc: { p: -0.12, y: -0.10, r: 0, z: -0.10, h: 0.06 },
  },
  /* 鎖鎌・右: 分銅を叩きつける。**一番大きく上下する。**
     鎖の先を回して落とす動きなので、振りかぶりを深く取る */
  'kusarigama.heavy': {
    time: 0.62, wind: 0.40, speed: 2.6, fade: 0.44,
    back: { p: 0.88, y: -0.24, r: 0.40, z: 0.16, h: 0.14 },
    thru: { p: -0.92, y: 0.20, r: -0.30, z: -0.10, h: -0.16 },
    arc: { p: -0.14, y: 0.14, r: 0.18, z: 0.14, h: -0.04 },
  },

  /* チェーンソー・左: 押し当てる。**振らない。**
     刃が自分で動く道具なので、当てて留めるのが正しい。
     前へ出す量は他より小さく、代わりに小刻みに震える見せ方（arcのh）にする */
  'chainsaw.light': {
    time: 0.34, wind: 0.20, speed: 3.0, fade: 0.56,
    back: { p: -0.10, y: 0.08, r: -0.06, z: 0.08, h: 0.03 },
    thru: { p: 0.06, y: -0.06, r: 0.05, z: -0.14, h: -0.03 },
    arc: { p: 0.05, y: 0.03, r: -0.04, z: -0.03, h: 0.05 },
  },
  /* チェーンソー・右: 横へ薙ぎ払う。**重い機械を振り回す。**
     捻り(r)を大きく取って、バーの面が返るのを見せる */
  'chainsaw.heavy': {
    time: 0.60, wind: 0.36, speed: 2.8, fade: 0.44,
    back: { p: -0.14, y: 0.86, r: -0.48, z: 0.14, h: 0.06 },
    thru: { p: 0.22, y: -0.96, r: 0.34, z: -0.06, h: -0.08 },
    arc: { p: -0.10, y: 0, r: 0.32, z: 0.16, h: 0.05 },
  },
...
/* 木目。**2026-08-12に足した。**
   「猟銃みたいなやつはコンセプトはいいんだけど、テキスチャーがなんか気持ち悪い。
   もうちょい木っぽくしてほしい」と言われた所。

   原因は**木に樹脂の肌を貼っていた**こと。walnutはSURF_POLYMER（等方な梨地）を
   法線の強さ1.5で使っていて、粒が全方向に均一に散った表面になっていた。
   等方な粒は石か樹脂で、**木は繊維が一方向に走っている**のが見分けの全部。

   ここでやるのは2つ:
     ・**縞を伸ばす。** xを0.16まで縮めてyを6倍にすると、
       同じノイズが縦へ長く引き伸ばされて筋になる
     ・**年輪を重ねる。** sinの位相へ木目のゆらぎを足して、
       等間隔の縞にしない（等間隔だと印刷した木目シートに見える）
   法線の強さは1.4で取る。木は金属ほど鋭い凹凸を持たないが、
   導管（細い溝）があるので布よりは立てる */
const SURF_WOOD = bakeSurface(96,
  (u, v) => {
    // 繊維。uを潰してvを伸ばす＝縦に長い筋になる
    const grain = fbm(u * 0.16, v * 6.0, 10, 3, 71);
    // 年輪。位相を繊維で揺らして、真っ直ぐな縞にしない
    const ring = Math.sin((v * 9 + grain * 4.0) * Math.PI) * 0.5 + 0.5;
    // 導管の細い点。木目の中に散る短い線
    const pore = fbm(u * 0.5, v * 3.0, 26, 2, 13);
    return grain * 0.42 + Math.pow(ring, 2.6) * 0.44 + pore * 0.14;
  },
  (u, v, h) => 0.78 + h * 0.26,
  1.4);
...
export const MATS = {
  ...
  /* **2026-08-12にSURF_POLYMERからSURF_WOODへ変えた。**
     樹脂の梨地を法線1.5で貼っていて、「テキスチャーが気持ち悪い」と言われた所。
     繰り返しを3から2へ落としてあるのは、**木目は間延びしている方が本物に見える**から
     （細かく繰り返すと、同じ節が銃床に何度も出て柄物の布になる）。
     法線の強さも1.5→0.7。木の導管は金属の削り跡ほど深くない */
  walnut: mat(0x4a2a18, 0.0, 0.72, SURF_WOOD, 2, 0.7, null,
  ...
  /* 星の粒と光る環（アストロ）。ドラゴンのemberと同じ作りで色だけ違う。
     **淡い白**にしてあるのは、サイバーの青緑・ヴェノムの黄緑と分けるため。
     星は色を持たない方が星に見える */
  starLight: new THREE.MeshBasicMaterial({
    color: new THREE.Color(2.6, 2.8, 3.4), toneMapped: false,
    transparent: true, opacity: 1, blending: THREE.AdditiveBlending, depthWrite: false,
  }),
  /* 竹（バンブー）。**ウエスタンの胡桃とは別の木。**
     あちらは磨いた焦茶の家具の木で、こちらは節のある若い竹。
     粗さを高く保って、艶を出さないのが分かれ目（磨いた木は光る） */
  // 竹も木目へ。**繰り返しは胡桃より細かい**（竹は繊維が細くて密）
  bamboo: mat(0xa8b060, 0.0, 0.68, SURF_WOOD, 4, 0.8, null,
    { amount: 0.40, color: 0xd0d494, metal: 0.0, rough: 0.58, dust: 0.14 }),

  /* ボクシンググローブの革。**strapでは代わりが効かない**
     （0x4a4e40の緑がかった帯なので、拳に使うと軍装の小物に見える）。
     赤茶の張りのある革にして、丸い塊として読めるようにする */
  mitt: mat(0x8c2a20, 0.0, 0.62, SURF_FABRIC, 4, 0.9, { envMapIntensity: 0.35 },
    { amount: 0.35, color: 0xc4584a, metal: 0.0, rough: 0.54 }),

  /* 刃文（はもん）。**日本刀と、ただの長い刃物を分ける一番大きい印。**
     焼き入れで硬くなった刃の側だけ結晶が細かくなって、白く曇った帯になる。

     地鉄(steel 0x5a6068・粗さ0.34)より明るく、粗さを上げて艶を落とす。
     **明るさだけ変えても刃文にならない**（磨いた鋼の反射が続くので、
     ただ明るい鋼に見える）。曇りは艶の差でしか出ない */
  hamon: mat(0xb4bcc2, 1.0, 0.52, SURF_METAL, 6, 0.35, { envMapIntensity: 0.45 },
    { amount: 0.35, color: 0xe4ecf2, rough: 0.44 }),
...
function addOptic(g, y, z) {
  ...
  for (const rz of [0.010, 0.034]) {
    g.add(part(torG(0.0347, 0.0035, 6, 26), MATS.anodized, 0, cy, z + rz));
  ...
  // 内壁。接眼側の直筒
  g.add(part(cylG(0.0300, 0.0300, 0.044, SEG, true), MATS.opticTube, 0, cy, z + 0.015, Math.PI / 2));
  // 内壁。対物ベル（前へ開く）
  g.add(part(cylG(0.0300, 0.0500, 0.035, SEG, true), MATS.opticTube, 0, cy, z - 0.0275, Math.PI / 2));
  // 外皮。内壁と分けると筒に厚みが出る。ここも必ず両端を開ける。
  // 蓋が付いていると視線の先に黒い円板が立って、覗いた瞬間に何も見えなくなる。
  // **ここは広げていない**（銃の太さを変えずに窓だけ広げるのが狙いなので）
  g.add(part(cylG(0.0330, 0.0330, 0.046, SEG, true), MATS.gunmetal, 0, cy, z + 0.014, Math.PI / 2));
  // 対物ベルの外皮。ADSでは硝子の周りの黒い輪の太さがそのままここで決まる。
  // 外径がレンズ半径の1.3倍もあると、等倍のダットサイトとしては遮蔽が大きすぎるので
  // 内壁(0.0340)ぎりぎりまで絞って、輪をレンズ半径の1.15倍に収める
  g.add(part(cylG(0.0330, 0.0526, 0.036, SEG, true), MATS.gunmetal, 0, cy, z - 0.028, Math.PI / 2));
...
function dragonDeco(g) {
  ...
  /* ---- 銃口の**竜の頭。2026-08-11に作り直した。**
     前は牙を上下に4本立てただけで、「先端をちょっとドラゴンっぽくするとか、
     もっとスキンとして魅力的な感じにしてほしい」と言われた所。

     牙だけでは**輪郭が筒のまま**なので、頭に見えない。
     頭骨・上下の顎・角2本・鼻先を組んで、**銃口そのものを頭にする。**
     使う長さは全長の1割（120mm）まで。
     **銃口の先は照準線の外**なので、そこは何を付けても構わない
     （赤ドットの円錐は照星より前へは効かない。窓はcheck-scopeの[4]が測る）*/

  // 頭骨。**一番大きい塊。**ここが無いと牙が浮く
  g.add(part(cboxG(0.058, 0.052, 0.090), MATS.scale, 0, R_BORE + 0.002, -0.626));
  // 鼻先。前へ絞る。四角い頭のままだと箱に見える
  g.add(part(cboxG(0.038, 0.034, 0.030), MATS.scale, 0, R_BORE - 0.002, -0.682));
  g.add(part(cboxG(0.024, 0.022, 0.016), MATS.scale, 0, R_BORE - 0.004, -0.700));

  /* ---- 上下の顎。**開いた口にする。** 上顎を持ち上げ、下顎を下げると
     「銃口が口の中」に見える。閃光はmuzzleの印から出るので位置は変えていない */
  for (const s of [1, -1]) {
    // 顎の板
    g.add(part(cboxG(0.046, 0.014, 0.062), MATS.scale, 0, R_BORE + s * 0.028, -0.678, s * 0.22));
    // 牙。**4本ずつに増やして、内側へ向ける**（外向きだと角に見える）
    for (let i = 0; i < 4; i++) {
      const x = -0.0165 + i * 0.011;
      const len = i === 1 || i === 2 ? 0.030 : 0.021;
      g.add(part(cylG(0, 0.0052, len, 5), MATS.bone,
        x, R_BORE + s * 0.021, -0.688, s > 0 ? Math.PI : 0));
    }
  }

  /* ---- 角。**2本、後ろへ反らせる。** 竜の角は後ろへ流れる
     （前へ向けると牛になる）。頭の上なので照準の円錐の外 */
  for (const s of [1, -1]) {
    g.add(part(cylG(0, 0.011, 0.072, 6), MATS.bone,
      s * 0.021, R_BORE + 0.036, -0.612, -0.70, s * 0.28, 0));
    // 角の節。1つ入れると生えている物に見える
    g.add(part(torG(0.0075, 0.0022, 4, 10), MATS.bone,
      s * 0.024, R_BORE + 0.048, -0.626, -0.70, s * 0.28, 0));
  }

  /* ---- 頭の鱗板。頬にあたる所へ2枚ずつ重ねる */
  for (const s of [1, -1]) {
    g.add(part(cboxG(0.008, 0.030, 0.044), MATS.scale, s * 0.031, R_BORE + 0.006, -0.634, 0, 0, s * 0.20));
    g.add(part(cboxG(0.008, 0.022, 0.032), MATS.scale, s * 0.026, R_BORE - 0.014, -0.660, 0, 0, s * 0.30));
  }
  ...
  /* ---- 目。**2つだけ。頭の上へ移した。**
     前は機関部の側面(z=-0.235)に置いていて、頭と離れていたので
     「銃に光る点が2つある」だけだった。頭の中に置くと**顔になる。**
     光る物を増やすと暗い場所で銃が提灯になるので、数は増やさない */
  for (const s of [1, -1]) {
    g.add(part(sphG(0.0105), MATS.ember, s * 0.023, R_BORE + 0.020, -0.646));
  }
  // 口の奥の熾火。牙の間から覗く。頭を大きくしたぶん奥へ送る
  g.add(part(sphG(0.013), MATS.ember, 0, R_BORE, -0.668));
}
...
/* ライフルの桜。**2026-08-11に作り直した。**
   最初は漆の黒に金の桜という和の器に寄せた作りにしたが、
   「桜なんだから、もっと白とピンクをベースにしてほしい」と言われた。
   **「和」ではなく「桜」を作る**のが要件だった。

   もう1つ、飾りが小さすぎた。花びらが半径8.5mmで銃の全長が1193mm——
   **銃の0.7%の物を貼っていただけ**なので輪郭が1mmも変わっていなかった。

   **2026-08-12にもう一度作り直した。**「桜、見た目キモすぎ。
   普通に桜の柄をrifle bodyにつけるだけでいいやろ、なんなのあの先端」と言われた。

   前は輪郭から変えにいって、**銃口に5枚の花弁を開かせていた**（直径60mmの花）。
   輪郭は確かに変わるが、変わった先が「銃の先に花が咲いている物」で、
   銃として成立しなくなっていた。**同じ失敗をキャンディでもやりかけた所。**

   今回は要件をそのまま採る。**柄（がら）は面に置く物で、輪郭から生やす物ではない。**
     ・銃口の花弁は全部やめる（輪郭は素のライフルのまま）
     ・花は面へ**貼る**。厚みを半径の1/5まで潰して、面から浮かせない
     ・貼る先は側面と天面＝**構えていても目に入る所**
   輪郭で主張しないぶん、地の白（skins.jsのsakura）と朱の房が仕事をする */
function sakuraDeco(g) {
  const rear = g.userData.rear;

  /* ---- 桜の柄。**面に貼る。**
     5枚の花弁を円く並べて、法線の向きへ潰す。
     潰す量(0.20)がこの飾りの肝で、ここを大きくすると
     面から生えた瘤になって、前の銃口の花と同じ物になる。

     @param face 'x' … 側面へ貼る（左右に潰す）／'y' … 天面へ貼る（上下に潰す） */
  const flower = (x, y, z, r, face) => {
    const flat = 0.20;
    for (let i = 0; i < 5; i++) {
      const a2 = (i / 5) * Math.PI * 2 + 0.3;
      const dx = Math.cos(a2) * r * 1.02;
      const dy = Math.sin(a2) * r * 1.02;
      g.add(face === 'x'
        ? partS(sphG(r * 0.62, 8, 6), MATS.candyPink, x, y + dy, z + dx, flat, 1, 1)
        : partS(sphG(r * 0.62, 8, 6), MATS.candyPink, x + dx, y, z + dy, 1, flat, 1));
    }
    // 花の芯。金を1粒だけ置くと、花びらの集まりが1輪にまとまる
    g.add(face === 'x'
      ? partS(sphG(r * 0.26, 8, 6), MATS.charm, x, y, z, flat, 1, 1)
      : partS(sphG(r * 0.26, 8, 6), MATS.charm, x, y, z, 1, flat, 1));
  };
  /* 側面（機関部と先台）に大小4輪、天面に1輪。
     **左右の両面に置く。** 片側だけだと、逆から見た時に無地の白い銃になる */
  for (const s2 of [1, -1]) {
    flower(s2 * 0.0322, R_BORE + 0.004, -0.300, 0.026, 'x');
    flower(s2 * 0.0322, R_BORE - 0.008, -0.412, 0.019, 'x');
    flower(s2 * 0.0262, R_BORE + 0.010, -0.120, 0.022, 'x');
  }
  flower(0, R_RAIL - 0.014, -0.250, 0.024, 'y');

  /* ---- 螺鈿の帯。**白い地に映える細い線。**前より太くして読めるようにする */
  for (const s2 of [1, -1]) {
    g.add(part(boxG(0.003, 0.010, 0.180), MATS.ivory, s2 * 0.0262, R_BORE + 0.016, -0.045));
  }

  /* ---- 花吹雪。**銃床から流す。**
     散った花びらを1枚ずつ、大きさと角度を変えて並べる。
     rearの中へ入れるので、構えると一緒に消える（目の後ろの部品なので正しい）*/
  if (rear) {
    const petals = [
      [0.034, 0.020, 0.150, 0.014, 0.5], [-0.030, 0.038, 0.196, 0.011, -0.9],
      [0.038, -0.014, 0.232, 0.013, 1.2], [-0.036, 0.010, 0.268, 0.010, -0.4],
      [0.030, 0.032, 0.300, 0.012, 0.8],
    ];
    for (const [x, y, z, r, rz] of petals) {
      rear.add(partS(sphG(r, 8, 6), MATS.candyPink, x, y, z, 1, 0.28, 1.4, 0.4, 0, rz));
    }
    // 銃床の白い被せ。地が白いことを一番大きい面で出す
    rear.add(part(cboxG(0.050, 0.100, 0.168), MATS.ivory, 0, -0.020, 0.230, 0.10));
  }

  /* ---- 朱の紐と房。**桜色の隣に置く赤。** 白とピンクの中で唯一の濃い色になる */
  g.add(part(cylG(0.0030, 0.0030, 0.052, 6), MATS.vermilion, 0.026, -0.150, 0.046, 0.24));
  g.add(part(cylG(0.0095, 0.0065, 0.032, 8), MATS.vermilion, 0.030, -0.190, 0.052));
 ...
 */
/* 拳銃のサイレンサー。**2026-08-12に足した。**
 * 「ピストルに、サイレンサー付きのやつ欲しいな」と言われた所。
 *
 * **拳銃は輪郭でしか差が付かない。** 塗り替えが届く面が12.6%しかないので、
 * 色と小さい飾りで解こうとしたクローム（明るさ）とスケルトン（透明）は
 * 2つとも「微妙」で終わって、同じ日に棚から下げている。
 * 消音器は**前へ11cm伸びる筒**で、銃の全長が1.6倍になる＝一番大きく形が変わる飾り。
 *
 * **照準線は塞がない。** 筒の芯は銃身と同じ高さ(P_BORE)で半径18mmなので、
 * 天面は P_BORE+0.018＝照門の天面(P_BORE+0.0255)より下を通る。
 * 実測は tools/check-scope.mjs の[2.5]（塞ぐ物が素のままと同じ1個であること）。
 *
 * **銃口の印も前へ送る。** 送らないと、閃光と煙が筒の途中から出る
 */
function suppressorDeco(g) {
  const Z0 = -0.168;          // 消音器の後端（銃身の先）

  /* ---- ねじ切りの見える銃身。**筒を外した時にここが残る、が読める** */
  g.add(part(cylG(0.0088, 0.0088, 0.016, 12), MATS.steel, 0, P_BORE, Z0 + 0.012, Math.PI / 2));
  for (let i = 0; i < 4; i++) {
    g.add(part(torG(0.0090, 0.0009, 4, 12), MATS.steel, 0, P_BORE, Z0 + 0.006 + i * 0.004));
  }

  /* ---- 消音器の胴。**太くて長い。** ここが輪郭の全部 */
  /* 長さは88mm。**これ以上伸ばせない。**
     形違いは「元の武器の一番長い辺の1.5倍まで」で見張ってあって
     （tools/check-skins.mjs。構えは目の前なので、少し盛るだけで画面を覆う）、
     112mmで作ったら1.58倍になって落ちた */
  g.add(part(cylG(0.0180, 0.0180, 0.088, 18), MATS.phosphate, 0, P_BORE, Z0 - 0.044, Math.PI / 2));
  // 前後の座金。1本の棒に見えないよう、両端に段を作る
  g.add(part(cylG(0.0192, 0.0192, 0.010, 18), MATS.anodized, 0, P_BORE, Z0 - 0.006, Math.PI / 2));
  g.add(part(cylG(0.0192, 0.0192, 0.010, 18), MATS.anodized, 0, P_BORE, Z0 - 0.082, Math.PI / 2));
  /* 胴の溝。**等間隔で6本。** 無地の筒は「黒い棒」にしか見えないので、
     光の当たる線を作る（放熱の溝は実物にもある） */
  for (let i = 0; i < 6; i++) {
    g.add(part(torG(0.0184, 0.0016, 4, 16), MATS.enamel, 0, P_BORE, Z0 - 0.020 - i * 0.010));
  }
  // 前の口。中を暗くして「穴が抜けている」を出す
  g.add(part(cylG(0.0120, 0.0120, 0.012, 14, true), MATS.opticTube, 0, P_BORE, Z0 - 0.084, Math.PI / 2));
  g.add(part(torG(0.0150, 0.0022, 5, 16), MATS.steel, 0, P_BORE, Z0 - 0.088));

  /* ---- 銃口の印を筒の先へ送る。**閃光と煙の出所。**
     送り忘れると、消音器の途中から火が出る */
  g.userData.muzzle.position.set(0, P_BORE, Z0 - 0.096);
}
 ...
 *
 * **2026-08-12に作り直した。**「星と竹が全然だわ。コンセプトもないし、
 * スキンとしてのかっこよさもない」と言われた所。
 *
 * 前の物は紺紫の地に**光る粒を12個散らしただけ**で、
 * 「星」という言葉から連想した物を貼っただけになっていた。
 * 粒は形を持たないので、輪郭は素の狙撃銃のまま——
 * つまり**遠くから見ると何のスキンなのか分からない。**
 *
 * 今回は比喩を最後まで通す。**狙撃銃と天体望遠鏡は、
 * 「筒を覗いて遠くを見る道具」という一点で本当に同じ物**なので、
 * ここは他の武器では成立しない見立てになる（狙撃銃だけが持てる）。
 *   ・銃身を**白い鏡筒**で包む（19世紀の屈折望遠鏡の見た目）
 *   ・対物側に**真鍮の口金**と、その先へ黒い**フード（遮光筒）**
 *   ・横に**ファインダー（小さい副望遠鏡）**を平行に載せる。これが一番効く
 *   ・**赤経環（目盛りの環）**と**微動ハンドル**を機関部の横へ
 * 光るのは対物レンズ1枚だけにする（覗いた先に星がある、という意味）。
 *
 * 望遠照準の視界を塞がないのはアイスと同じ制約で、
 * 機関部の上（レール0.042〜光軸0.072の間）は空けたまま。
 * 太い物は全部**レールより下**か、**銃口より前**へ置いてある。
 */
function astroDeco(g) {
  const rear = g.userData.rear;

  /* ---- 鏡筒。**この飾りの本体。** 露出した銃身を白い筒で包む。
     半径30mmは銃身(15.5mm)と溝(15mm)を飲み込む太さで、
     **太いことが望遠鏡の条件**（細いと銃身にカバーを掛けただけに見える）*/
  g.add(part(cylG(0.030, 0.030, 0.400, 18), MATS.ivory, 0, S_BORE, -0.630, Math.PI / 2));
  // 筒の継ぎ目。等間隔の帯を2本。無地の白い筒はプラスチックの棒に見える
  for (const z of [-0.520, -0.740]) {
    g.add(part(cylG(0.0315, 0.0315, 0.014, 18), MATS.brass, 0, S_BORE, z, Math.PI / 2));
  }

  /* ---- 対物側の口金とフード。**銃口より前**なので、視線の円錐には入らない。
     フードは黒く、内側だけが見える向きに開けておく（遮光筒は中が黒い） */
  g.add(part(cylG(0.0335, 0.0335, 0.024, 18), MATS.brass, 0, S_BORE, -0.836, Math.PI / 2));
  g.add(part(cylG(0.0330, 0.0330, 0.070, 18, true), MATS.enamel, 0, S_BORE, -0.884, Math.PI / 2));
  /* 対物レンズ。**光る物はここ1枚だけ。**
     前は粒を12個散らしていて、暗い場所で銃が提灯になる形だった。
     1枚に絞ると「筒の奥が光っている」＝覗いた先に星がある、に読める */
  g.add(part(cylG(0.0275, 0.0275, 0.003, 18), MATS.starLight, 0, S_BORE, -0.846, Math.PI / 2));

  /* ---- ファインダー（副望遠鏡）。**望遠鏡だと一番分かるのがこれ。**
     本体の筒と平行に、細い筒を1本。左側へ出すのは
     右側だと構えた時に自分の腕で隠れるため。
     天面(y=0.036)はレール(0.045)より下なので照準には入らない */
  g.add(part(cylG(0.0105, 0.0105, 0.190, 12), MATS.brass, -0.046, S_BORE + 0.008, -0.380, Math.PI / 2));
  g.add(part(cylG(0.0130, 0.0130, 0.010, 12), MATS.brass, -0.046, S_BORE + 0.008, -0.470, Math.PI / 2));
  // ファインダーを載せる脚。浮いていると後から貼った物に見える
  for (const z of [-0.300, -0.450]) {
    g.add(part(cboxG(0.030, 0.008, 0.014), MATS.chrome, -0.036, S_BORE + 0.008, z, 0, 0, Math.PI / 2));
  }

  /* ---- 赤経環。**目盛りの環。** 望遠鏡は目盛りで星を探す道具なので、
     刻みのある環が1つあるだけで「計器」に見える */
  g.add(part(torG(0.0322, 0.0030, 5, 20), MATS.brass, 0, S_BORE, -0.430));
  for (let i = 0; i < 12; i++) {
    const a = (i / 12) * Math.PI * 2;
    g.add(part(boxG(0.0020, 0.0060, 0.0030), MATS.enamel,
      Math.cos(a) * 0.0330, S_BORE + Math.sin(a) * 0.0330, -0.430, 0, 0, a));
  }

  /* ---- 微動ハンドル。望遠鏡を少しずつ動かすつまみ。
     機関部の左（排莢口の反対側）へ。刻みを入れて掴む物に見せる */
  g.add(part(cylG(0.0060, 0.0060, 0.038, 10), MATS.chrome, -0.042, 0.000, -0.030, 0, 0, Math.PI / 2));
  g.add(part(cylG(0.0135, 0.0135, 0.012, 12), MATS.brass, -0.062, 0.000, -0.030, 0, 0, Math.PI / 2));
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2;
    g.add(part(boxG(0.0035, 0.0030, 0.014), MATS.brass,
      -0.062, Math.sin(a) * 0.0135, -0.030 + Math.cos(a) * 0.0135, 0, 0, a));
  }

  // 銃床。頬当てに星を2つだけ残す（rearの中へ。構えると一緒に消える）
  if (rear) {
    rear.add(part(sphG(0.0034, 6, 5), MATS.starLight, 0.020, 0.058, 0.220));
    rear.add(part(sphG(0.0034, 6, 5), MATS.starLight, -0.018, 0.058, 0.256));
  }
}
 ...
 * スキンとしてのかっこよさもない」と言われた所。
 *
 * 前の物は**銃身に竹の筒を被せて節を並べただけ**で、
 * 出来上がりは「緑に塗った狙撃銃」だった。竹は材料であって物ではないので、
 * 材料をなぞっても何を持っているのかが言えない。
 *
 * 今回は**竹で作られた物**を持ってくる。和弓は
 * 「遠くの的を、静かに、1射で仕留める道具」で、狙撃銃と役目がそのまま重なる。
 * 竹という材料も残る（和弓は竹を貼り合わせて作る）。
 *   ・銃身の横に**弓の弧**を1本通す。**輪郭が変わるのはここ**
 *   ・弧の内側に**弦**を張る。これが無いと「曲がった棒」で終わる
 *   ・握りに**籐巻き**、弭（先端）に**朱の房**
 *   ・先台は竹の筒のまま残す（弓と同じ材料であることが繋がる）
 * 日本刀（黒漆と金）と系統を揃えてある。
 *
 * 弓は**左へ出す。** 右だと構えた時に自分の腕で隠れる。
 * 望遠照準の制約はアイス・星と同じで、レール(0.045)より上へは何も出さない
 * （弓の芯はy=S_BORE=0.020、一番高い所でも0.036）
 */
function bambooDeco(g) {
  const rear = g.userData.rear;

  /* ---- 弓の弧。**この飾りの本体。**
     短いカプセルを繋いで曲線にする（押し出しより軽く、継ぎ目も出ない）。
     和弓は握りが下から1/3にある**左右非対称の弓**なので、
     一番膨らむ所を真ん中ではなく前寄り(t=0.56)に置いてある。
     ここを真ん中にすると洋弓になって、和弓の見分けが付かない */
  const BOW_X = -0.040;     // 弦を張る面（先台の外側。銃に食い込ませない）
  const BOW_D = 0.048;      // 一番膨らむ量
  const at = (t) => {
    const z = -0.090 - t * 0.760;
    // 山を前寄りへ寄せた膨らみ。両端で0になる
    const k = Math.sin(Math.PI * Math.min(1, Math.max(0, t)) ** 1.35);
    return [BOW_X - BOW_D * k, S_BORE, z];
  };
  for (let i = 0; i < 10; i++) {
    const r = 0.0072 - Math.abs(i - 4.5) * 0.0004;   // 端へ行くほど細る
    g.add(capBetween(MATS.bamboo, at(i / 10), at((i + 1) / 10), r));
  }
  /* ---- 弦。**まっすぐ張る。** 弧と弦の間に隙間ができて初めて弓に見える
     （弧だけだと、曲がった竹が横に付いているだけ）*/
  g.add(capBetween(MATS.ivory, at(0), at(1), 0.0018));
  // 弭（ゆはず）。弦を掛ける先端の金具。両端に1つずつ
  for (const t of [0, 1]) {
    const p = at(t);
    g.add(part(sphG(0.0068, 8, 6), MATS.enamel, p[0], p[1], p[2]));
  }

  /* ---- 籐巻き（とうまき）。握りと、竹を貼り合わせた継ぎ目に巻く黒漆の帯。
     和弓は全長に何十本も巻いてあるが、ここは読める数だけ置く */
  for (const t of [0.30, 0.34, 0.38, 0.62, 0.66]) {
    const p = at(t);
    const q = at(t + 0.012);
    g.add(capBetween(MATS.enamel, p, q, 0.0082));
  }

  /* ---- 朱の房。弭に下げる。**白と黒と竹色の中で唯一の色。**
     桜（朱の房）と日本刀（金）と同じ系統に揃える */
  const tip = at(1);
  g.add(part(cylG(0.0085, 0.0050, 0.030, 8), MATS.vermilion, tip[0], tip[1] - 0.024, tip[2]));

  /* ---- 先台の竹。**弓と同じ材料であることが繋がる。**
     角張った台を丸い筒で覆う。節を3本入れて竹だと分かるようにする */
  g.add(part(cylG(0.0270, 0.0270, 0.280, 12), MATS.bamboo, 0, S_BORE - 0.004, -0.300, Math.PI / 2));
  for (const z of [-0.400, -0.300, -0.200]) {
    g.add(part(cylG(0.0285, 0.0285, 0.014, 12), MATS.bamboo, 0, S_BORE - 0.004, z, Math.PI / 2));
  }

  /* ---- 麻紐の巻き。**握る所に。** 竹は割れるので縛って補強する物で、
     紐が無いと「竹を模した樹脂」に見える */
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.0195, 0.0195, 0.010, 10), MATS.strap, 0, S_BORE, -0.150 + i * 0.013, Math.PI / 2));
  }

  // 銃床。頬当てを竹で包む（rearの中へ）
  if (rear) {
    rear.add(part(cylG(0.0210, 0.0210, 0.110, 12), MATS.bamboo, 0, 0.056, 0.230, Math.PI / 2));
    rear.add(part(cylG(0.0225, 0.0225, 0.012, 12), MATS.bamboo, 0, 0.056, 0.180, Math.PI / 2));
  }
}
...
/* ---------------------------------------------- リボルバーとソードオフ
 *
 * 2026-08-12に足した2つ。「ピストルがなんかな。どれもイケてないんだよな、スキン。
 * これ欲しいみたいなのがなくて」「ショットガンもなんか好きに足んないなぁ」と言われた。
 *
 * **色や飾りでは届かない、と判断した所。** 拳銃は塗り替えが届く面が12.6%しかなく、
 * そこへ色を5つ並べても「同じ銃の色違い」から出られない。
 * 短剣で形違い（刀・斧・レイピア）が一番効いているのと同じで、
 * **持った時に別の銃になる**のが一番強い。
 *
 * どちらも元の銃の枠（射程・威力・弾数・装填の種類）はそのまま。
 * 変えるのは組み立てだけで、強さには1つも触っていない。
 */

/* リボルバー。**スライドを無くすのが本体。**
   今の拳銃は「スライド・フレーム・握把・照準の4つの塊」で読ませているので、
   一番大きい塊(スライド)を丸い銃身と回転弾倉へ入れ替えると輪郭が丸ごと変わる。
   足す物より**外す物**で決まる形違い */
function buildRevolver(view = {}) {
  const g = new THREE.Group();
  const grip = view.grip || {};
  const TILT = -0.42;        // 握把の傾き。自動拳銃(-0.30)より寝かせる

  /* ---- 銃身。**丸い。** 上に細いリブを通して、平らな面を1つも作らない */
  g.add(part(cylG(0.0110, 0.0110, 0.150, 16), MATS.steel, 0, P_BORE, -0.100, Math.PI / 2));
  g.add(part(boxG(0.009, 0.006, 0.150), MATS.steel, 0, P_BORE + 0.013, -0.100));
  // 排莢桿（はいきょうかん）の覆い。銃身の下の細い筒。ここが無いと棒に見える
  g.add(part(cylG(0.0070, 0.0070, 0.120, 12), MATS.phosphate, 0, P_BORE - 0.018, -0.090, Math.PI / 2));
  g.add(part(cylG(0.0050, 0.0050, 0.016, 10), MATS.steel, 0, P_BORE - 0.018, -0.158, Math.PI / 2));

  /* ---- 回転弾倉。**この銃の顔。**
     溝(フルート)を6本彫る。無地の円柱だと「太い所」にしかならない */
  g.add(part(cylG(0.0235, 0.0235, 0.050, 14), MATS.phosphate, 0, P_BORE - 0.001, 0.008, Math.PI / 2));
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2;
    g.add(part(boxG(0.0075, 0.0075, 0.038), MATS.enamel,
      Math.cos(a) * 0.0225, P_BORE - 0.001 + Math.sin(a) * 0.0225, 0.008, 0, 0, a));
  }
  // 弾倉を留める芯と、後ろの薬莢の見える面
  g.add(part(cylG(0.0060, 0.0060, 0.062, 10), MATS.steel, 0, P_BORE - 0.001, 0.008, Math.PI / 2));
  for (let i = 0; i < 6; i++) {
    const a = (i / 6) * Math.PI * 2 + 0.5;
    g.add(part(cylG(0.0042, 0.0042, 0.004, 8), MATS.brass,
      Math.cos(a) * 0.0135, P_BORE - 0.001 + Math.sin(a) * 0.0135, 0.034, Math.PI / 2));
  }

  /* ---- 上帯（トップストラップ）。弾倉を跨いで機関部と銃身を繋ぐ梁。
     **照準線はこの上を通る**ので、ここより上には何も置かない */
  g.add(part(cboxG(0.017, 0.010, 0.070), MATS.phosphate, 0, P_BORE + 0.023, 0.004));
  // 機関部（フレーム）。弾倉の下を支える
  g.add(part(cboxG(0.026, 0.034, 0.080), MATS.phosphate, 0, P_BORE - 0.030, 0.012));
  g.add(part(cboxG(0.030, 0.028, 0.040), MATS.phosphate, 0, P_BORE - 0.014, 0.048));

  /* ---- 撃鉄。**立っているのが見えることがリボルバーの印。**
     ただし照準線(P_BORE+0.028)より上へ出さない。出すと覗いた時に的が隠れる */
  g.add(part(cboxG(0.008, 0.022, 0.013), MATS.steel, 0, P_BORE + 0.016, 0.056, 0.40));
  g.add(part(boxG(0.011, 0.007, 0.009), MATS.knurl, 0, P_BORE + 0.024, 0.060, 0.40));

  /* ---- 引金と用心鉄。丸い輪にする（自動拳銃の角ばった物と分ける） */
  g.add(part(torG(0.019, 0.0034, 6, 16, Math.PI), MATS.phosphate,
    0, P_BORE - 0.044, 0.030, 0, Math.PI / 2, Math.PI));
  g.add(part(boxG(0.006, 0.020, 0.006), MATS.steel, 0, P_BORE - 0.034, 0.030, 0.2));

  /* ---- 握把。**胡桃。** 今の拳銃5つは全部金属か樹脂なので、木がここだけになる */
  g.add(part(cboxG(0.028, 0.086, 0.042), MATS.walnut, 0, -0.060, 0.070, TILT));
  g.add(part(cboxG(0.030, 0.014, 0.044), MATS.brass, 0, -0.100, 0.084, TILT));
  // 握把の輪郭の丸み。角のままだと板を貼っただけに見える
  g.add(partS(sphG(1), MATS.walnut, 0, -0.024, 0.056, 0.014, 0.016, 0.021));

  /* ---- 照準。照星と、上帯に彫った照門 */
  g.add(part(boxG(0.004, 0.011, 0.005), MATS.phosphate, 0, P_BORE + 0.021, -0.168));
  g.add(part(boxG(0.007, 0.008, 0.008), MATS.steel, 0.008, P_BORE + 0.026, 0.030));
  g.add(part(boxG(0.007, 0.008, 0.008), MATS.steel, -0.008, P_BORE + 0.026, 0.030));

  /* ---- 銃側が必ず読む3つの印 */
  const muzzle = new THREE.Object3D();
  muzzle.position.set(0, P_BORE, -0.180);
  g.add(muzzle);
  g.userData.muzzle = muzzle;
  const eject = new THREE.Object3D();
  eject.position.set(0.024, P_BORE, 0.010);
  g.add(eject);
  g.userData.eject = eject;
  const sight = new THREE.Object3D();
  sight.position.set(0, P_BORE + 0.028, 0.004);
  g.add(sight);
  g.userData.sight = sight;
  ...
  handR.position.fromArray(grip.pos || [0, -0.052, 0.062]);
  handR.rotation.fromArray(grip.rot || [TILT, 0, 0]);
  g.add(handR);
  g.userData.handR = handR;
  ...
  handL.position.set(-0.020, -0.066, 0.072);
  handL.rotation.set(TILT, 0, 0.34);
  g.add(handL);
  g.userData.handL = handL;

  /* 装填中に添え手が辿る位置。**弾倉へ行く道筋にしてある。**
     自動拳銃の「握把の下から弾倉を挿す」をそのまま使うと、
     リボルバーなのに握把の底を触りに行く画になる */
  g.userData.holdL = {
    rest: [[-0.020, -0.066, 0.072], [TILT, 0, 0.34]],
    // 弾倉の横。ここで押し出しと詰め替えをしている形にする
    mag: [[-0.034, -0.020, 0.010], [0.10, 0.24, 0.30]],
    low: [[0.010, -0.230, 0.070], [0.46, 0.30, -0.24]],
    charge: [[-0.030, 0.012, 0.006], [0.34, 0.10, 0.44]],
  };
...
/* ソードオフ。**銃身を切る。**
   元のショットガンは全長1.1mの筒物で、放熱筒と弾倉チューブが輪郭の大半を作っている。
   そこを二連の短い銃身と中折れの機関部へ入れ替えると、
   **同じ武器とは思えない短さ**になる（切り詰めた握把がそれを言い切る）。

   銃床は持たない。切ってあるので、覗いた時に消える部品(rear)も要らない */
function buildSawedOff(view = {}) {
  const g = new THREE.Group();
  const grip = view.grip || {};
  const B = 0.022;           // 銃身の芯。元のショットガンと同じ高さに合わせる
  const SY = 0.050;          // 照準線。二連の間のリブより上を通す
  const BX = 0.019;          // 左右の銃身の芯の間隔（の半分）

  /* ---- 二連の銃身。**横に並べる。**
     上下2連にすると輪郭が縦に伸びて、切り詰めた感じが出ない */
  for (const s of [1, -1]) {
    g.add(part(cylG(0.0175, 0.0175, 0.250, 18), MATS.phosphate, s * BX, B, -0.160, Math.PI / 2));
    g.add(part(torG(0.0180, 0.0026, 5, 16), MATS.steel, s * BX, B, -0.283));
  }
  // 2本の間の溝を埋めるリブ。上下に1本ずつ通すと「二連の銃身」が1つの塊になる
  g.add(part(boxG(0.030, 0.007, 0.250), MATS.phosphate, 0, B + 0.012, -0.160));
  g.add(part(boxG(0.030, 0.007, 0.250), MATS.phosphate, 0, B - 0.012, -0.160));
  // 切り口。**切ったことが分かる面。** 地金を出して、そこだけ光らせる
  g.add(part(boxG(0.074, 0.036, 0.006), MATS.steel, 0, B, -0.286));

  /* ---- 機関部。中折れの蝶番と、開けるためのレバー */
  g.add(part(cboxG(0.062, 0.064, 0.115), MATS.enamel, 0, 0.004, 0.010));
  g.add(part(cylG(0.0090, 0.0090, 0.066, 12), MATS.steel, 0, B - 0.030, -0.038, 0, 0, Math.PI / 2));
  addStampX(g, 0.0315, -0.002, 0.010, 0.038, 0.022, 3);
  addStampX(g, -0.0315, -0.002, 0.010, 0.038, 0.022, 3);
  // 上のレバー。親指で横へ倒して開く物。照準線より下で止める
  g.add(part(cboxG(0.026, 0.008, 0.024), MATS.steel, 0, 0.038, 0.036));

  /* ---- 露出した撃鉄。**2つ並ぶのが二連の印。**
     照準線(0.050)より上へ出さない。出すと覗いた時に的が隠れる */
  for (const s of [1, -1]) {
    g.add(part(cboxG(0.008, 0.022, 0.012), MATS.steel, s * 0.014, 0.036, 0.062, 0.42));
    g.add(part(boxG(0.011, 0.007, 0.009), MATS.knurl, s * 0.014, 0.044, 0.066, 0.42));
  }

  /* ---- 先台。**木。** 短い銃身に木の先台が付くと猟銃の出自が出る */
  g.add(part(cboxG(0.050, 0.030, 0.115), MATS.walnut, 0, B - 0.030, -0.130));
  g.add(part(cylG(0.0060, 0.0060, 0.052, 10), MATS.brass, 0, B - 0.044, -0.130, 0, 0, Math.PI / 2));

  /* ---- 引金（動く）。二連なので引金も2つ並べる */
  const trg = new THREE.Group();
  trg.position.set(0, -0.028, 0.058);
  trg.add(part(boxG(0.007, 0.024, 0.007), MATS.steel, -0.007, -0.012, -0.004, 0.2));
  trg.add(part(boxG(0.007, 0.024, 0.007), MATS.steel, 0.007, -0.012, 0.008, 0.2));
  g.add(trg);
  g.userData.trigger = trg;
  g.add(part(torG(0.022, 0.0036, 6, 16, Math.PI), MATS.enamel,
    0, -0.036, 0.056, 0, Math.PI / 2, Math.PI));

  /* ---- 握把。**切り詰めてある。** 銃床の付け根で切った形にして、
     木の断面をそのまま残す（ここが「ソードオフ」の言葉そのもの） */
  g.add(part(cboxG(0.040, 0.098, 0.052), MATS.walnut, 0, -0.076, 0.104, -0.36));
  g.add(part(cboxG(0.042, 0.014, 0.054), MATS.steel, 0, -0.120, 0.120, -0.36));
  for (let i = 0; i < 3; i++) {
    g.add(part(boxG(0.042, 0.005, 0.010), MATS.enamel, 0, -0.048 - i * 0.022, 0.088 + i * 0.008, -0.36));
  }
  ...
  /* ---- 照準。真鍮のビーズ1つだけ。切ってある銃に照門は付かない */
  g.add(part(sphG(0.0042, 8, 6), MATS.brass, 0, B + 0.016, -0.276));
  const sight = new THREE.Object3D();
  sight.position.set(0, SY, 0.040);
  g.add(sight);
  g.userData.sight = sight;

  const muzzle = new THREE.Object3D();
  muzzle.position.set(0, B, -0.292);
  g.add(muzzle);
  g.userData.muzzle = muzzle;
  const eject = new THREE.Object3D();
  eject.position.set(0.030, 0.020, 0.010);
  g.add(eject);
  g.userData.eject = eject;

  /* ---- 手。右は握把、左は先台。**ポンプが無いので左は動かない** */
  const handR = buildHand(1, {
    gripR: 0.022, wrap: 0.50, trigger: true,
    armDir: grip.armDir || [0.38, -0.62, 0.94], armLen: 0.62,
  });
  handR.position.set(0.008, -0.052, 0.092);
  handR.rotation.set(-0.36, 0, 0);
  g.add(handR);
  g.userData.handR = handR;
  ...
  handL.position.set(0, -0.014, -0.130);
  handL.rotation.set(-Math.PI / 2 + 0.08, 0, 0.10);
  g.add(handL);
  g.userData.handL = handL;

  g.userData.holdL = {
    rest: [[0, -0.014, -0.130], [-Math.PI / 2 + 0.08, 0, 0.10]],
    // 中折れなので、装填口は機関部の上。そこへシェルを落とす
    mag: [[0.024, -0.010, 0.010], [0.20, 0.26, -0.40]],
    low: [[0.048, -0.230, 0.070], [0.48, 0.36, -0.36]],
    charge: [[0, -0.014, -0.130], [-Math.PI / 2 + 0.08, 0, 0.10]],
  };
...
function buildLeverShotgun(view = {}) {
  ...
  /* ---- 機関部。**真鍮の一枚板。**
     最初は青い鋼にしていたが、「使っている感じでウエスタンとの違いが
     あんまわかんない」と言われた（2026-08-15）。構えた画面の下半分に
     常に見えているのは機関部なので、ここを金色にして横目でも分かる差にする
     （実物にも真鍮機関部のレバー銃がある。ウィンチェスター1866）。
     ウエスタンの真鍮は輪や口金の小物だけなので、面の大きさで被らない */
  g.add(part(cboxG(0.044, 0.060, 0.170), MATS.brass, 0, 0.002, 0.000));
  g.add(part(cboxG(0.040, 0.014, 0.150), MATS.brass, 0, 0.032, 0.000));
  ...
  /* ---- 銃身と弾倉チューブ。放熱筒は付けない
     ...
     **銃身は八角。** 覗いた時に目の前へ来るのはここなので、
     丸（元）とも穴あきの放熱筒（ウエスタン）とも角の立ち方で分ける。
     材質もsteelにして、塗りで青光りする側へ振る */
  g.add(part(cylG(0.0180, 0.0180, 0.430, 8, true), MATS.steel, 0, B, -0.330, Math.PI / 2, Math.PI / 8));
  ...
  /* ---- レバーの輪っか（動く）。**この銃の一番の見せ場。**
     機関部の下、引金の後ろへ縦の輪を置く。トリガーガードを兼ねる形にして、
     別のガードは付けない（輪が2つ並ぶと何の銃か読めなくなる）。

     **撃つたびに前へ振り出して戻る。** 最初は止めて出したが、
     「使っている感じでウエスタンとの違いがあんまわかんない」と言われた。
     ウエスタンは撃つたびにポンプが動くのに、こちらは光るだけだったので、
     使っている間の差が出ていなかった。拍はポンプ(pumping)と同じ物を使い、
     滑らせる代わりに回す（駆動は_poseWeaponのP.lever） */
  const lev = new THREE.Group();
  lev.position.set(0, -0.030, 0.096);   // 蝶番。機関部の下端
  lev.add(part(torG(0.032, 0.0042, 6, 18), MATS.steel, 0, -0.028, -0.014, 0, Math.PI / 2, 0));
  // 輪と機関部を繋ぐ柄。宙に浮いた輪はレバーに見えない
  lev.add(part(boxG(0.009, 0.026, 0.014), MATS.steel, 0, -0.006, 0.008));
  g.add(lev);
...
//
// **2026-08-12に日本刀へ寄せ直した。**「刀は日本刀って名前にして、
// もっと日本刀ぽくしておいて」と言われた所。
// 前の物は「反りのある長い片刃＋円い鍔」までしか作っていなくて、
// そこから先の**日本刀にしか無い物**が1つも無かった。足したのは4つ:
//   ・刃文（はもん）… 焼き入れの白い曇り。**これが一番効く**
//   ・切先（きっさき）… 横手で折れる三角の先。前は峰から刃へ直に繋いでいた
//   ・鎺（はばき）… 鍔の前の真鍮の襟。刀を鞘に留める金具
//   ・鮫皮と目貫 … 柄巻きの菱形の隙間から覗く白と、握りの金具
function buildKatana(view = {}) {
  ...
  /* 反りは「先へ行くほど峰側（-x）へ寄る」で出す。
     ...
     **切先（きっさき）を作る。** 峰の先から刃側へ直に戻すと、
     先が細く尖るだけの「長いナイフ」になる。日本刀の先は
     横手（よこて）という段で一度折れて、そこから三角に立ち上がる。
     刃側に折れ点を1つ置くだけで、その三角が出る */
  const YOKO_X = BW * 0.30 - SORI * 0.92;   // 横手（刃側の折れ点）
  const YOKO_Y = -BL * 0.905;
  const TIP_X = -BW * 0.15 - SORI;          // 切っ先
  const blade = new THREE.Shape();
  blade.moveTo(-BW, 0);                                   // 根元・峰側
  blade.quadraticCurveTo(-BW - SORI * 0.45, -BL * 0.55, -BW - SORI * 0.92, -BL * 0.93);
  blade.lineTo(TIP_X, -BL);                               // 切っ先
  blade.lineTo(YOKO_X, YOKO_Y);                           // 横手まで戻る＝切先の三角
  // 刃側を根元へ戻る。峰と同じだけ反らせないと、先が太って鉈に見える
  blade.quadraticCurveTo(BW * 0.62 - SORI * 0.5, -BL * 0.5, BW, 0);
  blade.closePath();
  ...
  /* 刃文（はもん）。**日本刀にしか無い物の筆頭。**
     焼き入れで硬くなった刃の側だけ白く曇る。境目は波打っていて（互の目）、
     直線で引くと**焼き入れではなく塗装に見える。**

     刃の側の線は刃そのものと同じ曲線をなぞり、峰の側は波で戻す。
     刃より少しだけ厚く押し出して、両面から0.4mm出す（＝Zファイトを避ける）。
     数字は刃の曲線を0.2刻みで解いて、そこから6〜16mm峰側へ寄せた点 */
  const hamon = new THREE.Shape();
  hamon.moveTo(BW, -BL * 0.02);
  hamon.quadraticCurveTo(BW * 0.62 - SORI * 0.5, -BL * 0.5, YOKO_X, YOKO_Y);
  hamon.splineThru([
    new THREE.Vector2(-0.0273, -0.222),   // 深い所（谷）
    new THREE.Vector2(-0.0101, -0.171),   // 浅い所（山）
    new THREE.Vector2(-0.0126, -0.118),
    new THREE.Vector2(0.0051, -0.063),
    new THREE.Vector2(0.0050, -BL * 0.02),
  ]);
  hamon.closePath();
  g.add(part(bladeGeoFrom(hamon, 0.0073, 0.0008, -0.030, 0.0034), MATS.hamon, 0, 0, 0));

  /* 鎺（はばき）。鍔の前に嵌める真鍮の襟。**刀を鞘に留めている金具。**
     細い部品だが、刃と柄の間に1つ挟まっているだけで
     「作りのある刀」に見える（西洋剣にはこれが無い） */
  g.add(part(cboxG(0.045, 0.013, 0.026), MATS.brass, 0, 0.001, -0.044));

  /* 鍔（つば）。**円盤1枚。** 刀と他の刃物を分ける一番大きい印がこれで、
     ここが四角いと途端に西洋剣に見える */
  g.add(part(cylG(0.030, 0.030, 0.006, 18), MATS.phosphate, 0, 0.002, -0.026, Math.PI / 2));
  // 覆輪（ふくりん）。鍔の縁の金の輪。板1枚だと座金に見えるので縁を立てる
  g.add(part(torG(0.030, 0.0024, 5, 20), MATS.brass, 0, 0.002, -0.026, 0, 0, 0));
  ...
  /* 鮫皮（さめがわ）。**柄巻きの菱形の隙間から白く覗く。**
     実物の柄は白いエイの皮を巻いた上から紐を掛けるので、
     交差と交差の間に白い菱形が並ぶ。**紐だけだと黒い棒**で、
     ここに白が入って初めて日本刀の柄に見える（象牙の材質を使い回している） */
  for (let i = 0; i < 5; i++) {
    const z = 0.0105 + i * 0.021;
    g.add(part(boxG(0.009, 0.003, 0.009), MATS.ivory, 0, 0.0178, z, 0, Math.PI / 4, 0));
    // 側面にも。構えると柄は54度傾いて見えるので、上面だけだと白が隠れる
    g.add(part(boxG(0.003, 0.009, 0.009), MATS.ivory, 0.0118, 0.002, z, Math.PI / 4, 0, 0));
    g.add(part(boxG(0.003, 0.009, 0.009), MATS.ivory, -0.0118, 0.002, z, Math.PI / 4, 0, 0));
  }
  // 目貫（めぬき）。柄の側面の金具。指が掛かる所に付ける小さな飾り
  g.add(partS(sphG(1), MATS.brass, 0.0135, 0.002, 0.058, 0.004, 0.007, 0.016));
  g.add(partS(sphG(1), MATS.brass, -0.0135, 0.002, 0.058, 0.004, 0.007, 0.016));
  // 頭（かしら）。柄尻の金具
 ...
 *
 * 短剣の形違いは5つあったが、**全部が「刃物か拳」**だった
 * （日本刀・ダガー・レイピア・斧・グローブ）。
 * この2つは**手に持つ物の系統そのものが違う**:
 *   鎖鎌 … 刃と分銅が鎖で繋がっている＝2つの塊が離れて付いている
 *   チェーンソー … 機械。刃が自分で動く物
 *
 * **強さは今まで通り全部同じ。** 威力も間隔も間合いも変えない
 */

/* 鎖鎌。**鎌と鎖と分銅の3つで読む。**
   鎌だけだと「曲がったナイフ」で終わるので、
   柄尻から垂れる鎖と、その先の分銅までを1つの武器として出す */
function buildKusarigama(view = {}) {
  const g = new THREE.Group();

  /* ---- 柄。**木。** 鎌は農具なので、金属の柄にすると武器に寄りすぎる */
  g.add(part(cylG(0.0135, 0.0155, 0.190, 12), MATS.walnut, 0, 0.002, 0.028, Math.PI / 2));
  // 握る所の縄巻き。使う道具であることが出る
  for (let i = 0; i < 4; i++) {
    g.add(part(cylG(0.0168, 0.0168, 0.012, 10), MATS.strap, 0, 0.002, 0.038 + i * 0.017, Math.PI / 2));
  }
  // 柄と刃の継ぎ目の金具
  g.add(part(cylG(0.0175, 0.0175, 0.016, 12), MATS.phosphate, 0, 0.002, -0.062, Math.PI / 2));

  /* ---- 鎌の刃。**前へ出てから内へ大きく曲がる。**
     ここが真っ直ぐだと鉈になる。刃は内側（+x側）に付いていて、
     引き寄せて切る道具であることが輪郭から読める */
  const blade = new THREE.Shape();
  blade.moveTo(-0.010, -0.010);                            // 根元・背側
  blade.quadraticCurveTo(0.026, -0.120, 0.104, -0.150);    // 背。前へ出て内へ回る
  blade.lineTo(0.110, -0.132);                             // 切っ先
  blade.quadraticCurveTo(0.040, -0.108, 0.012, -0.010);    // 刃。内側を戻る
  blade.closePath();
  g.add(part(bladeGeoFrom(blade, 0.0060, 0.0026, -0.062, 0.003), MATS.steel, 0, 0, 0));
  /* 刃先の帯。**研いだ所だけ明るい。** 内側の弧に沿って細く1枚 */
  const edge = new THREE.Shape();
  edge.moveTo(0.108, -0.134);
  edge.quadraticCurveTo(0.040, -0.104, 0.013, -0.012);
  edge.quadraticCurveTo(0.044, -0.094, 0.104, -0.126);
  edge.closePath();
  g.add(part(bladeGeoFrom(edge, 0.0068, 0.0009, -0.062, 0.0028), MATS.chrome, 0, 0, 0));

  /* ---- 鎖。**柄尻から前へ回して、手の上を通す。**

     **2026-08-12に道筋を作り直した。**「普通に装備して歩いてる時の目線、
     せっかくのその鎖が見えてないから、そこは調整してほしいかも」と言われた所。

     前は柄尻から**真下へ垂らして**いた。測ったら鎖の面の**92%が画面の外**だった
     （柄尻そのものが画面(306,205)＝右下の枠の外にあるので、
     そこから下がる物は全部見えない）。

     今は柄尻から**前・左・上**へ回して、分銅を画面(186,138)へ置く。
     手に掛けて分銅を前へ垂らしている形で、
     **鎖の胴と分銅が画面の中で読める。**

     鎖は群れ(chain)に入れてある。**柄尻を原点に回せる**ようにするためで、
     武器を見る動き(5キー)でここを回すと分銅がぶんぶん振り回される
     （INSPECTSのkusarigama.spin） */
  const chain = new THREE.Group();
  /* 鎖を留める所。**柄と刃の継ぎ目の金具そのもの。**

     柄尻から移したのは、柄尻が画面(306,206)＝右下の枠の外にあって、
     **そこから下へ垂らすと何をしても見えない**ため（測った。長さ0.2で画面(299,319)）。
     鎖鎌は柄尻に留める物と刃の付け根に留める物の両方が実在するので、見える側を採る。

     **一度、刃の脇の何も無い所へ置いてしまった**（2026-08-12。
     「鎖のついてる位置がおかしい。つき始めてる根元」と言われた）。
     x=0.028 は刃の輪郭より外で、**金属が1つも無い空中**だった
     （刃の輪郭は付け根で x=0.012 までしか無い）。

     今は継ぎ目の金具（半径17.5mmの環）の芯へ置いてある。
     最初の輪が金具の中に隠れて、そこから下へ出る＝**留まっているように見える。**
     芯なら傾きが変わっても輪郭の外へ出ない */
  chain.position.set(0, 0.002, -0.062);
  const CH = 10;
  /* 付け根(原点)から下へ。**この群れは模型の回転を打ち消してある**ので、
     ここでの -y はそのまま「世界の下」になる（_animateの鎖の節）。
     真っ直ぐ引くと棒に見えるので、左右へわずかに蛇行させる。

     長さ0.092は**測って決めた。** 止まっている時に分銅が画面(245,186)＝
     下の縁のすぐ外に居て、走ると(245,174)まで上がって画面へ入る。
     「歩いたり走ったりしたらギリ見える」がこの数字 */
  const at = (t) => [
    Math.sin(t * 2.6) * 0.009,
    -0.092 * t,
    -0.010 * t,
  ];
  for (let i = 0; i < CH; i++) {
  ...
  /* ---- 分銅。**鎖の先の重り。** ここが無いと鎖がただ垂れているだけになる。
     **材質を刃と分けてある。** 同じMATS.steelにすると、
     押し出した刃（索引なし）と球（索引あり）が同じ束へ入って結合が失敗する。
  ...
  /* 鎖を留める環。**継ぎ目の金具の下**に付ける。
     金具の半径が17.5mmなので、その縁へ掛かる所へ置く */
  g.add(part(torG(0.0072, 0.0024, 5, 12), MATS.phosphate, 0, -0.016, -0.062, 0, Math.PI / 2, 0));
...
/* チェーンソー。**刃物ではなく機械。**
   短剣の形違いで唯一「自分で動く物」を持ってくる。
   角張った発動機の胴・前へ伸びるバー・上の取っ手・引き紐で読ませる。

   **バーは薄い。** 厚くすると板を持っているように見えるので、
   刃を並べた縁の方で存在を出す */
function buildChainsaw(view = {}) {
  const g = new THREE.Group();
  const grip = view.grip || {};
  const BY = 0.006;          // バーの芯の高さ

  /* ---- 発動機の胴。**角張った箱。** 丸くすると玩具に寄る */
  g.add(part(cboxG(0.070, 0.078, 0.120), MATS.polymer, 0, -0.004, 0.070));
  g.add(part(cboxG(0.060, 0.020, 0.124), MATS.polymer, 0, 0.040, 0.070));
  // 側面の抜きと、始動の引き紐の巻き取り
  for (const s of [1, -1]) {
    for (let i = 0; i < 3; i++) {
      g.add(part(boxG(0.006, 0.026, 0.014), MATS.enamel, s * 0.0355, -0.010, 0.036 + i * 0.024));
    }
  }
  g.add(part(cylG(0.026, 0.026, 0.014, 14), MATS.enamel, 0.038, -0.004, 0.070, 0, 0, Math.PI / 2));
  // 引き紐。輪だけ出す（垂らすと構えた時に手の中で暴れて見える）
  g.add(part(cylG(0.0022, 0.0022, 0.048, 6), MATS.strap, 0.048, 0.010, 0.070, 0.4));
  g.add(part(torG(0.0088, 0.0026, 4, 10), MATS.strap, 0.048, 0.036, 0.070, 0, Math.PI / 2, 0));
  // 排気の筒。機械であることが一番安く出る部品
  g.add(part(cylG(0.0105, 0.0105, 0.040, 10), MATS.phosphate, -0.036, 0.026, 0.040, 0, 0, Math.PI / 2));

  /* ---- 上の取っ手。**胴の上を跨ぐ弓。** 両手で持つ道具の印 */
  g.add(part(cboxG(0.016, 0.012, 0.096), MATS.polymer, 0, 0.070, 0.062));
  for (const z of [0.020, 0.106]) {
    g.add(part(cboxG(0.014, 0.030, 0.014), MATS.polymer, 0, 0.056, z));
  }

  /* ---- 前の取っ手とガード。**左手はここを握る。**
     ガード（前へ張り出した板）はチェーンソーの一番分かりやすい部品 */
  g.add(part(cylG(0.0105, 0.0105, 0.084, 10), MATS.polymer, 0, 0.026, -0.010, 0, 0, Math.PI / 2 + 0.2));
  g.add(part(cboxG(0.078, 0.030, 0.010), MATS.enamel, 0, 0.062, -0.026, 0.34));

  /* ---- バー。**前へ長く、薄く。** 全長の半分以上がこれになる */
  g.add(part(cboxG(0.012, 0.046, 0.290), MATS.phosphate, 0, BY, -0.180));
  // 先の丸み。角のままだと板を切り落としただけに見える
  g.add(part(cylG(0.023, 0.023, 0.011, 14), MATS.phosphate, 0, BY, -0.322, 0, 0, Math.PI / 2));
  // 溝。バーの真ん中に彫ってある線。無地の板から抜け出す
  g.add(part(boxG(0.014, 0.010, 0.286), MATS.enamel, 0, BY, -0.180));

  /* ---- 刃。**上下の縁に並べる。**
     ここが「切る物」であることの全部なので、数を惜しまない（材質は1つなので描画は1回）*/
  for (let i = 0; i < 13; i++) {
    const z = -0.056 - i * 0.021;
    g.add(part(cboxG(0.015, 0.011, 0.013), MATS.chrome, 0, BY + 0.026, z, 0.5));
    g.add(part(cboxG(0.015, 0.011, 0.013), MATS.chrome, 0, BY - 0.026, z, -0.5));
  }
  // 先端の刃。丸い所にも2つ置くと、輪が繋がって見える
  g.add(part(cboxG(0.015, 0.013, 0.011), MATS.chrome, 0, BY + 0.020, -0.334, 0.9));
  g.add(part(cboxG(0.015, 0.013, 0.011), MATS.chrome, 0, BY - 0.020, -0.334, -0.9));

  /* ---- 左手。**meleeRigは右手しか出さない**ので、前の取っ手を握る手をここで足す。
     グローブと同じやり方（あちらは両拳、こちらは前の取っ手） */
  const handL = buildHand(-1, {
    gripR: 0.019, wrap: 0.86, trigger: false,
    armDir: grip.armDirL || [-0.44, -0.72, 0.60], armLen: 0.56,
  });
  handL.position.set(-0.010, 0.024, -0.012);
  handL.rotation.set(-Math.PI / 2 + 0.12, 0, 0.24);
  g.add(handL);

  meleeRig(g, view, { muzzleZ: -0.360, sightZ: -0.170, gripR: 0.019 });
  bakeStatic(g);
  return g;
}
...
export const SHAPE_BUILDS = {
  ...
  suppressed: (view = {}) => buildPistol(view, suppressorDeco),
};
...
/* 斧の頭を、**真横から見た輪郭**から起こす道具。
   刃物(bladeGeoFrom)とは寝かせる向きが違う。

   刃物は「上から見た形」を寝かせて、厚みを上下(Y)に出す＝刃が水平に寝る。
   斧はそれでは組めない。**斧は横から見た形で読む物**で、
   厚み（頬）は左右(X)に出る。ここを刃物と同じ向きで組んでいたせいで、
   前の斧は「柄の先に前へ伸びた平たい板が付いた物」になっていた
   （2026-08-12に「斧が全然斧に見えない」と言われた所）。

   rotateY(+90度)で (x, y, z) → (z, y, -x)。つまり
     図形のx … 前（-Z）へ    図形のy … 上（+Y）へ    押し出し … 左右（X）へ
   厚みは0から+Xへ伸びるので、半分だけ戻して芯に乗せる */
function axeGeoFrom(shape, depth, bevel, z = 0, y = 0) {
  const geo = new THREE.ExtrudeGeometry(shape, {
    depth, bevelEnabled: true,
    bevelThickness: bevel * 0.73, bevelSize: bevel, bevelSegments: 1,
  });
  geo.rotateY(Math.PI / 2);
  geo.translate(-depth / 2, y, z);
  return geo;
}

/* 斧。**刃渡りで測る物ではない。**
   長さは柄が作っていて、大きさは「刃頭の弧」が作る。

   **2026-08-12に作り直した。**「斧が全然斧に見えない」と言われた所。
   前の物は刃物(ナイフ・刀)と同じ組み方で、
     ・頭が柄と同じ向きに前へ伸びていた（＝柄の先に付いた幅広の刃＝薙刀）
     ・厚みが上下にあって、頬が上下を向いていた
     ・柄の先端(z=-0.140)より頭(z=-0.180〜)が前にあって、繋がっていなかった
   の3つが重なっていた。斧は**柄を横切って頭が載る**道具なので、
   ここを直さないと何を足しても斧に見えない。

   今の組み方:
     ・頭は真横から見た輪郭で作り、頬は左右を向く（axeGeoFrom）
     ・刃は柄から前下がりに張り出す。振り下ろした時に**刃が先に当たる**向き
     ・柄は頭の眼(socket)を貫いて、少しだけ先へ出る */
function buildAxe(view = {}) {
  const g = new THREE.Group();

  // 頭の中心（眼）を置く位置。ここを基準に前後(z)を測る
  const EYE = -0.145;

  /* 刃頭。**顎(ひげ)のある片刃。** 図形は「横から見た形」で、
     +xが前、+yが上。原点は柄の芯＝眼の中心。

     刃の弧は前上の角から前下へ回して、顎で柄の側へ戻す。
     この弧が斧の輪郭のほぼ全部で、直線で結ぶと鉈になる */
  const head = new THREE.Shape();
  head.moveTo(-0.046, 0.020);                                   // 峰（打撃面）の上・後ろ
  head.lineTo(-0.014, 0.024);                                   // 眼の上
  /* **一度くびれてから、爪先(toe)へ跳ね上がる。**
     ここを直線で結ぶと首が太いままで、包丁の輪郭になる。
     斧は「細い首から扇が開く」形 */
  head.quadraticCurveTo(0.030, 0.012, 0.088, 0.046);
  head.quadraticCurveTo(0.100, 0.004, 0.084, -0.026);           // 刃・上半分。前へ膨らむ
  head.quadraticCurveTo(0.074, -0.064, 0.032, -0.090);          // 刃・下半分。顎の先へ
  head.quadraticCurveTo(0.030, -0.048, 0.000, -0.028);          // 顎の内側。柄の側へ抉れる
  head.lineTo(-0.014, -0.024);
  head.lineTo(-0.046, -0.018);                                  // 峰の下・後ろ
  head.closePath();
  g.add(part(axeGeoFrom(head, 0.017, 0.0032, EYE), MATS.steel, 0, 0.002, 0));

  /* 刃先の帯。**研いだ所だけ明るい。**
     一枚板だと鉄の板にしか見えないので、弧に沿って細い面を1枚重ねる。
     頭より厚く押し出して、両頬から少しはみ出させる（＝研ぎ落とした縁） */
  const edge = new THREE.Shape();
  edge.moveTo(0.088, 0.046);
  edge.quadraticCurveTo(0.100, 0.004, 0.084, -0.026);
  edge.quadraticCurveTo(0.074, -0.064, 0.032, -0.090);
  edge.quadraticCurveTo(0.064, -0.060, 0.074, -0.024);          // 内側へ戻る＝帯の幅
  edge.quadraticCurveTo(0.088, 0.004, 0.080, 0.042);
  edge.closePath();
  g.add(part(axeGeoFrom(edge, 0.019, 0.0010, EYE), MATS.chrome, 0, 0.002, 0));

  /* 眼（柄が通る穴）の外周。**柄より太くないと、柄が頬から飛び出して見える。**
     頭の板は厚さ17mmしかないのに柄は直径31mmあるので、
     ここで包まないと板の両側から柄がはみ出す。
     長さは頭の板の奥行きぶん取る（前は52mmで、峰の側で柄が飛び出していた）。
     実物も眼の周りは肉厚で、ここが1段太いことで「頭が柄に載っている」が読める */
  g.add(part(cylG(0.020, 0.020, 0.075, 12), MATS.phosphate, 0, 0.002, EYE + 0.012, Math.PI / 2));
  // 楔。眼の上に打ち込んで頭を留める物。真鍮で1つ
  g.add(part(cboxG(0.014, 0.010, 0.024), MATS.brass, 0, 0.028, EYE));

  /* 木の柄。**眼を貫いて少し先へ出す。** 突き抜けていないと、
     頭が宙に浮いて別部品に見える（前の作りがそれだった）。
     手元へ向かって細くなる（手斧は頭の側が太い） */
  g.add(part(cylG(0.0155, 0.0125, 0.300, 10), MATS.walnut, 0, 0.002, -0.020, Math.PI / 2));
  ...
  g.add(part(torG(0.0135, 0.0028, 5, 12), MATS.brass, 0, 0.002, 0.106));

  // 刃は柄の先の下。閃光の出所も刃の少し先で止める
  meleeRig(g, view, { muzzleZ: -0.250, sightZ: -0.130, gripR: 0.022 });

  /* **構えの角度を上書きする。** 形を直しただけでは足りなかった所。
     斧は「頭の面」で読む物なのに、ナイフと同じ構え（真っ直ぐ前へ突き出す）だと
     **頭の面が視線と平行**になって、画面上では線にしかならない。
     組み直した後も「角度的に全然斧に見えない」と言われたのがこれ。

     **測って決めてある。** 画面(16:9・画角55度)に刃頭が占める割合:
       ナイフと同じ構え [0.02, 0.04, 0.95] … 0.62%（刀の刃は1.23%、拳は4.55%）
       この構え         [-0.15, 0.98, 0.22] … 2.40%
     横へ回して面をこちらへ向けると4倍近くになる。
     照準のまわり（中心から8度）は0画素のまま＝狙う邪魔をしない */
  g.userData.hip = [0.24, -0.18, -0.40];
  g.userData.hipRot = [-0.15, 0.98, 0.22];
 ...
 *
 * **2026-08-11に作り直した。** 最初は革の甲に13mmの当て金を4つ並べた作りで、
 * 「全然パンチグローブに見えない。見た目がどうにもならない」と言われた。
 *
 * 原因は**大きさ。** 当て金13mmに対して模型は929mm。銃の1.4%の物を貼っていたので、
 * 輪郭が素手のままだった。**ボクシンググローブは「丸い塊」で読む物**なので、
 * 拳を包む volume を作らないと何を足しても素手に見える。
 *
 * 今回は拳を半径55mmの膨らんだ革で包む（前の4倍）。
 * 親指の膨らみと手首の太い帯も作って、**輪郭が丸くなる。**
 *
 * **両拳を出す。** 片手だけ構えていると
 * 「何かを握っているのに手が空いている」形になる。
 * ただし左手は独立して動かない（振りは模型ごと動く作りなので、
 * 腕が武器の群れの中に居るのと同じ制約）。
 */
function buildGlove(view = {}) {
  ...
  /* ---- 右のミット。**これが武器の本体。**
     潰した球で拳を包む。半径55mmで、指の側を前へ伸ばす。

     **群れ(fistR)に入れる。** 2026-08-12に左右交互で打つようにしたので、
     打っていない方をその場へ置いておくには**両方が別々の群れ**でないといけない
     （_animateが、打っていない側だけを逆へ戻す）*/
  const fistR = new THREE.Group();
  fistR.add(partS(sphG(0.055, 14, 10), MATS.mitt, 0, 0.000, -0.036, 1.0, 0.94, 1.15));
  /* 拳の前。**2026-08-12に平らな板から丸い塊へ変えた。**
     前は cboxG(0.086, 0.076, 0.026) を先端に貼っていて、
     「先端に板みたいのついてる」と言われた所。実物のグローブは
     指の側が一番丸いので、板を貼ると**そこだけ機械の部品**に見える */
  fistR.add(partS(sphG(0.050, 14, 10), MATS.mitt, 0, -0.004, -0.070, 1.0, 0.92, 0.78));
  // 親指の膨らみ。これが無いと左右対称の玉になって、ボールに見える
  fistR.add(partS(sphG(0.028, 10, 8), MATS.mitt, -0.046, -0.014, -0.046, 1.0, 0.9, 1.3, 0, 0.3, 0));
  // 縁取り。**ボクシンググローブは白い縁で読む。** 指の側をぐるりと回す
  fistR.add(part(torG(0.049, 0.0042, 6, 22), MATS.ivory, 0, -0.002, -0.062, 0, 0, 0));
  // 紐（レース）。手首の甲に3本。**金属を1つも使わない**
  for (let i = 0; i < 3; i++) {
    fistR.add(part(boxG(0.026, 0.004, 0.005), MATS.ivory, 0, 0.028, 0.006 + i * 0.013));
  }
  // 手首の帯。太く巻く。一番後ろだけ布にして革との継ぎ目を作る
  for (let i = 0; i < 3; i++) {
    fistR.add(part(cylG(0.036, 0.034, 0.018, 14), MATS.mitt, 0, -0.004, 0.014 + i * 0.020, Math.PI / 2));
  }
  fistR.add(part(cylG(0.037, 0.037, 0.010, 14), MATS.strap, 0, -0.004, 0.048, Math.PI / 2));

  /* ---- 左のミット。**meleeRigは右手しか作らない**ので、こちらで足す。

     **左右それぞれ別の群れ(fistR / fistL)。** 2026-08-12に
     「左クリックは交互で右左でパンチするようにしてほしいわ。左右左右で」で
     この形になった。振りは模型ごと動かす作りなので、
     群れに分けておかないと両方の拳が同じだけ前へ出る
     （_animateが、打っていない側だけを逆へ戻す）。

     **置き方は左右で揃えてある。**「なんか左右のバランスおかしい」と言われた所で、
     前は右が画面(251,156)で近く、左が(250,183)＝**画面の下の外**に居た。
     しかも半径が右55mm・左46mmで違った。今は同じ大きさで、
     画面の(251,156)と(108,158)＝中心を挟んでほぼ対称に置いてある */
  const fistL = new THREE.Group();
  const handL = buildHand(-1, {
    gripR: 0.024, wrap: 0.92, trigger: false,
    armDir: grip.armDirL || [-0.42, -0.62, 0.86],
    armLen: 0.58,
  });
  handL.position.set(-0.206, 0.280, -0.024);
  handL.rotation.set(-0.24, 0.18, -0.20);
  fistL.add(handL);
  g.userData.handL = handL;
  // 右と同じ大きさ。**片方だけ小さいと遠近が壊れて、奥に引っ込んで見える**
  fistL.add(partS(sphG(0.055, 14, 10), MATS.mitt, -0.210, 0.276, -0.064, 1.0, 0.94, 1.15));
  fistL.add(partS(sphG(0.050, 14, 10), MATS.mitt, -0.210, 0.272, -0.098, 1.0, 0.92, 0.78));
  fistL.add(partS(sphG(0.028, 10, 8), MATS.mitt, -0.166, 0.262, -0.074, 1.0, 0.9, 1.3, 0, -0.3, 0));
  fistL.add(part(torG(0.049, 0.0042, 6, 22), MATS.ivory, -0.210, 0.274, -0.090, 0, 0, 0));
  for (let i = 0; i < 3; i++) {
    fistL.add(part(boxG(0.026, 0.004, 0.005), MATS.ivory, -0.210, 0.304, -0.022 + i * 0.013));
  }
  for (let i = 0; i < 3; i++) {
    fistL.add(part(cylG(0.036, 0.034, 0.018, 14), MATS.mitt, -0.210, 0.272, -0.014 + i * 0.020, Math.PI / 2));
  }
  fistL.add(part(cylG(0.037, 0.037, 0.010, 14), MATS.strap, -0.210, 0.272, 0.020, Math.PI / 2));

  /* 閃光の出所と覗きの寄せ先は一番手前。**拳は届く距離が短く見える**方が正しい
     （当たり判定は間合いで決まっていて形に効かないので、見た目の話）*/
  meleeRig(g, view, { muzzleZ: -0.140, sightZ: -0.080, gripR: 0.024 });
  /* **右手を右の拳の群れへ移す。** meleeRigは手を g へ足すので、
     ここで移さないと「ミットは止まるのに手だけ前へ出る」ことになる。
     add()は元の親から外してくれるので、外す処理は要らない */
  if (g.userData.handR) fistR.add(g.userData.handR);
  /* 群れの中も結合する。**bakeStaticは直下の面しか見ない**ので、
     ここで呼ばないと拳の面が1枚ずつ残る（実際32個まで増えた）*/
  bakeStatic(fistR);
  bakeStatic(fistL);
  g.add(fistR);
  g.add(fistL);
  g.userData.fistR = fistR;
  g.userData.fistL = fistL;
  bakeStatic(g);
  return g;
}
...
export const WEAPONS = [
  {
    ...
    view: {
      ...
      scale: 0.86, adsScale: 0.64, adsDist: 0.096,
  ...
  },
  {
    ...
    /* 2026-08-11に78から105へ上げた（「連射速度をもうちょっと高めてもいいよ」）。
       1発の間隔が0.77秒→0.57秒。**ポンプを引く動き(pumpTime 0.45)より長い所で止めてある**
       ——ここを0.45より短くすると、引き終わる前に次が出て動きが飛ぶ。

       **威力は上げていない。** 1発13×9粒のまま。
       近距離で当たった時の強さは変わらず、外した後の立て直しが速くなる */
    damage: 13, headMult: 1.6, rpm: 105, auto: false, pellets: 9,
  ...
  },
  {
    ...
    /* **1人用では胴に当てれば1発で倒れる。** 2026-08-12に足した。
       「スナイパーを使う旨味があんまりないかもしれん」と言われた所で、
       弾が全部で10発しかない銃なのに、波が進むと胴2発になっていた
       （敵の体力は100＋波×12で最大220、この銃の威力は110）。

       **威力の数字は上げていない。** 上げても波の伸びにいつか負けるので、
       埋めるのは1人用の射撃解決の側（src/main.jsの_resolveShot）。
       頭と脚は今まで通りで、胴だけが確実に落ちる */
    soloOneShot: true,
...
class Weapon {
  ...
  constructor(def, viewScene, plain = false) {
    ...
    /* 構えの位置と角度。**形違いのスキンだけが上書きできる。**
       普通は武器の表(def.view)が持っている物で、形違いは形だけを変える。
       ただし斧のように**面で読む物**は、銃と同じに前へ突き出すと
       刃が視線と平行になって画面上では線にしかならない
       （実測: 刃頭が画面の0.62%。組み直した後も角度が同じで
         「角度的に全然斧に見えない」と言われた。上書きして2.40%になった）。
       書いていない形は今まで通り武器の構えで出る */
    const hp = this.parts.hip || v.hip;
    const hr = this.parts.hipRot || v.hipRot;
    this.hipPos = new THREE.Vector3(hp[0], hp[1], hp[2]);
    this.hipRot = new THREE.Euler(hr[0], hr[1], hr[2]);
...
export class WeaponSystem {
  ...
  _startSwing(kind) {
    ...
    /* **左右の拳を交互に出す。** パンチグローブだけが持つ挙動で、
       2026-08-12に「左クリックは交互で右左でパンチするようにしてほしいわ。
       左右左右で」と言われた所。

       持っているのは「今どちらで打っているか」の1つだけ。
       左クリック(light)のたびに入れ替える。
       右クリック(heavy)は大きなフックなので、**必ず右**（体ごと回す動きを左右で
       作り分けると、腕が武器の群れに入っている都合で片方が破綻する）*/
    if (w.parts.fistL) this.punchLeft = kind === 'light' ? !this.punchLeft : false;
  }
  ...
  _animate(dt, input, player) {
    ...
    /* ---- 打っていない方の拳を、その場へ置いておく。
       **パンチグローブだけが持つ群れ（offhand）。**

       2026-08-12に「クリックでどっちも前に出るのおかしい、片側だけにして」
       と言われた所。振りは模型ごと動かす作りなので、
       **両手が同じだけ前へ出ていた**（拳の武器では、それが「両手で突き出す」に見える）。

       模型が前へ出た分だけ、この群れを逆へ戻す。
       群れは縮尺(v.scale)の中にいるので、動かす量もそのぶん割る。
       **回転は打ち消していない。** 振りの回転は9度ほどで、
       15cm離れた拳が動くのは2cm強。前後の32cmに比べて誤差の範囲だし、
       構え全体がわずかに連れて動く方が「体で打っている」に見える */
    const fistR = w.parts.fistR;
    const fistL = w.parts.fistL;
    if (fistR && fistL) {
      const k = 1 / (v.scale || 1);
      // 打っていない方だけ、模型が前へ出た分を逆へ戻す
      const idle = this.punchLeft ? fistR : fistL;
  ...
  _animateParts(dt, w, d) {
    ...
    /* ---- レバーの輪っか（レバーアクション）。拍はポンプと同じ物を使い、
       滑らせる代わりに**前へ振り出して戻す**。滑る輪っかは
       「回して装填する銃」に見えない。1.1rad（63度）で指1本ぶん残して戻る */
    if (P.lever) {
      let po = 0;
      if (this.pumping > 0 && d.pumpTime) po = Math.sin((1 - this.pumping / d.pumpTime) * Math.PI);
      P.lever.rotation.x = po * 1.1;
    }
  ...
  refillReserve() {
    let got = false;
    /* 数字キーの4本＋Qの1本＋Eの1本。**持っている枠を全部見る。**
       Qの物を外すと訓練場で狙撃銃だけ弾が戻らず、
       Eの物(rangeIndex)を外すと**ショットガンだけ弾が戻らない**
       （2026-08-11にEの枠を足した時にここへ書き足すのを忘れて、
         2026-08-12に「ショットガンだけは弾が無限じゃない」と言われた。
         同じ抜け方を2回やっているので、tools/check-range.mjsで見張っている）*/
    for (const i of [...this.carry, this.quickIndex, this.rangeIndex]) {
      if (i == null) continue;
      const w = this.weapons[i];
