...
export const MATS = {
  ...
  /* 透ける樹脂（スケルトン）。**中が見える物は品揃えに1つも無い。**
     氷柱(icicle)と同じ理由で depthWrite を切ってある——
     重なった透明が互いの奥を消して、板を挿したように見えるのを防ぐ。
     opacityを0.34まで落としてあるのは、**中の発条と弾を見せる**のが目的なので、
     枠が濃いとせっかく入れた中身が隠れる */
  clearPoly: new THREE.MeshStandardMaterial({
    color: 0xd8e4ea, metalness: 0.0, roughness: 0.12,
  ...
  bamboo: mat(0xa8b060, 0.0, 0.68, SURF_POLYMER, 6, 1.1, null,
...
/* 拳銃のスケルトン。**中が見える。品揃えに1つも無い見せ方。**
 *
 * サイバー（光）・クローム（明るさ）に対して、こちらは**透明。**
 * 拳銃は塗り替えが届く面が12.6%しかないので面積で勝負できないが、
 * 「透けている」は面積ではなく**性質**なので、小さくても目を引く。
 *
 * 照準線（後ろz=0.026 → 前z=-0.128）を跨がないのは他の形と同じ。
 * **透明な物は検査の判定から外れる**（レンズやレティクルと同じ扱いで
 * 「覗く邪魔にならない物」として数えない）が、それに頼らず線の外へ置く。
 */
function skeletonDeco(g) {
  /* ---- 透ける枠。スライドとフレームの側面に貼る。
     **元の部品を消さずに重ねる**（消すと形を覚え直す必要が出る）。
     透けているので、下の元の部品がそのまま中身として見える */
  for (const s of [1, -1]) {
    g.add(part(cboxG(0.003, 0.026, 0.166), MATS.clearPoly, s * 0.0158, P_BORE + 0.002, -0.050));
    g.add(part(cboxG(0.003, 0.016, 0.116), MATS.clearPoly, s * 0.0136, -0.006, -0.040));
  }
  // 天面の透ける板。照門(y=P_BORE+0.0255)より低く止める
  g.add(part(cboxG(0.026, 0.003, 0.150), MATS.clearPoly, 0, P_BORE + 0.016, -0.056));

  /* ---- 中に見える発条（ばね）。**光らせる。2026-08-11に変えた。**
     前は磨いた鋼で置いていて、「音も微妙だし、見た目も変わり映えない」と言われた。
     透ける枠の中に**暗い金属**を入れても、外から見て何も起きていない。

     熾火(ember)を使い回している。**サイバーの青緑と分かれる**のが選んだ理由で
     （同じ拳銃で光る物が2つあると、どちらも「光る銃」で終わる）、
     暖色の橙は「機械が動いて熱を持っている」に読める。

     輪も太くした（0.0014→0.0030）。細い輪は透ける枠の中では見えない */
  for (let i = 0; i < 7; i++) {
    g.add(part(torG(0.0070, 0.0030, 5, 10), MATS.ember,
      0, P_BORE + 0.010, -0.104 + i * 0.016, Math.PI / 2));
  }
  /* ---- 中に見える弾。**握把の中に3発、光らせる。**
     透ける枠の中で光る物なので、これが「中が見える」の合図になる */
  for (let i = 0; i < 3; i++) {
    g.add(part(cylG(0.0058, 0.0058, 0.024, 10), MATS.ember,
      0, -0.056 - i * 0.016, 0.036 + i * 0.005, P_GRIP_TILT, 0, Math.PI / 2));
  }
  // 撃鉄の発条。銃尾に1つ。ここも光らせて、後ろから見ても中が見える
  g.add(part(torG(0.0080, 0.0026, 5, 12), MATS.ember, 0, P_BORE + 0.008, 0.030, Math.PI / 2));
  /* 銃身の中の光。**筒の中を通す。** 覗いた時に銃口の奥が光るので、
     「中で何かが起きている」が一番はっきり出る所 */
  g.add(part(cylG(0.0034, 0.0034, 0.070, 8), MATS.ember, 0, P_BORE, -0.112, Math.PI / 2));

  /* ---- 枠の縁。透明だけだと輪郭が消えるので、細い金属で縁取る。
     **透ける物は縁が無いと「何も無い」に見える** */
  for (const s of [1, -1]) {
    g.add(part(boxG(0.002, 0.003, 0.164), MATS.chrome, s * 0.0164, P_BORE + 0.015, -0.050));
    g.add(part(boxG(0.002, 0.003, 0.164), MATS.chrome, s * 0.0164, P_BORE - 0.011, -0.050));
  }
}
