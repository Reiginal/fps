...
/* **モデルを置いてある武器の名前。ここに書いた物しか読みに行かない。**
   ...
   **今は空。**（2026-08-07）その後スナイパー寄りの形だと言われ、
   同じ配布パックのak47へ差し替えてみたが、**今度は太らせ処理と噛み合わなかった。**
   ak47はマガジンが斜めに大きく張り出す形で、fitModel()の太らせ(FIT_FAT)は
   「断面はだいたい棒状」という前提（長い向き以外の2軸を同じ倍率で太らせる）で
   組んである。マガジンぶんまで含めて太らせると、全体が黒い塊になり、
   手からも浮いて見えた。**FIT_FATの数値を変えて直る話ではなく、
   非対称な形と今の当てはめ方式の相性の問題。** 詳しくはCREDITS.md */
