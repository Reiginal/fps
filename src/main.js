    ...
    switch (ev.e) {
      ...
      case EV.KILL: {
        ...
        if (ev.id === me) {
          this._tally('deaths');
          this.streak = 0;
          /* **何で倒されたかまで出す。** 2026-08-13に
             「ガンゲームで相手がまだアサルト持ってるはずなのに、ショットガン」と
             言われた所。あれは**観戦の画が「今その人が持っている物」を出している**ためで、
             ガンゲームでは倒した瞬間に段が進んで武器が替わっている。

             画そのものは正しい（生きている人の今の姿を見ている）ので、
             直すのは**こちらが何で倒されたかを言っていなかった**方。
             倒された電文には使われた武器(ev.w)が入っているので、それを出す */
          const byW = WEAPONS[ev.w | 0];
          const how = byW ? `${byW.nick || byW.name}で` : '';
          this.hud.banner('ラウンドを落とした', `${net.nameOf(ev.by)}に${how}倒された`, 1.8);
        }
        break;
      }
      ...
      case EV.ARM: {
        ...
        /* **段のある遊び方（ガンゲーム）では手榴弾を数えない。**
           2026-08-13に「ガンゲームの時は、手榴弾の弾の制限があったらちょっと
           おかしいよね」で足した。サーバー側も同じ扱いにしてある
           （server/modes.jsのnadeLimit）。

           段の数で見分けているのは、**遊び方の名前をここまで運んでいない**ため。
           段が2つ以上ある＝進んでいく遊び方＝ガンゲームしか無い */
        this.weapons.nadeFree = (ev.of | 0) > 1;
        if (this.weapons.nadeFree) this.weapons.addNades(NADE.PER_ROUND);
        break;
      }
