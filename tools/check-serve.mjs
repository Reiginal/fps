...
// 個人情報の扱い。メールアドレスを預かる以上、誰でも読めないと意味が無い。
// ゲーム本体とは別の1枚なので、名前を足し忘れると404になる
serves('/privacy.html', '個人情報の扱い');

// 画面のエラーを受け取る口。ここは配る物ではないので、
// 配ってよいURLの判定からは外れていること（server/index.jsが先に横取りする）。
// 判定を通ってしまうと、/report という名前のファイルを置けば外から読めることになる
blocks('/report', 'ファイルとしては配らない');

// 会員証の口も同じ。server/index.js が静的配信より手前で横取りする。
// 判定を通ってしまうと、api という名前のフォルダを置けば外から中身が読める
blocks('/api/me', 'ファイルとしては配らない');
blocks('/api/login');
blocks('/api/register');
