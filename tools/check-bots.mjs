...
console.log('\n[6] 強すぎない（振り向きざまの即死をしない）');
{
  ...
  /* 出会い頭に必ず負けないよう、人が反応するのと同じくらいの間を置く。
     ...
     **下限は2026-08-11に30刻み(0.5秒)から60刻み(1.0秒)へ上げた。**
     「マジで強すぎる。macのトラックパッドだから無理」と言われて
     反応(REACT_S)と振り向き(TURN_RATE)を大きく落としたので、そのぶんを固定する。
     上げないと、次に誰かが数字を戻しても検査が通ってしまう。

     トラックパッドは指の可動域が狭くて振り向きに時間がかかるので、
     「速さ」で差が付く項目はこちらが必ず負ける。だから人の反応(0.25秒前後)より
     はっきり遅い所まで持っていく */
  ok(firstFire > 60, `真後ろの相手を撃つまで${firstFire}刻み（1.0秒より長い）`);
...
console.log('\n[7] 反動の代わりの散りと、連射の区切り');
{
  ...
  /* **散りの伸びしろにも下限を置く。** 2026-08-11に上限を0.085→0.170へ広げたぶん。
     ここが狭いと、押しっぱなしのCPUが全弾当ててくる状態に戻る。
     0.10radは20m先で2mぶん散る量で、当て続けられなくなる所 */
  ok(Math.max(...devs) > 0.10,
    `撃ち続けると十分に散る（一番外れた弾で${Math.max(...devs).toFixed(3)}rad / 0.10より上）`);
}
