   ...
   熱に変わる。同じ値で呼んでも2度目からは触らないこと */
{
  ...
  // 地図。**毎フレーム塗り直さない**（塗るたびに絵を画面へ送り直すことになる）
  {
    ...
    /* 塗るのは必ず間引きの後。
       ...
       **文字数の窓で見るのをやめた。** 前は「_mapAccからN文字以内にhud.minimap」で
       見ていたので、間に処理やコメントが増えるたびに窓を広げる作業が要った
       （200→1200→…）。見たいのは距離ではなく**順番**だけなので、
       出てくる位置を比べる。これなら間に何行入っても、
       塗りが間引きより前に出た時だけ落ちる */
    const guardAt = main.indexOf('this._mapAcc < 1 / MAP_HZ');
    const paintAt = main.indexOf('this.hud.minimap(');
    ok(guardAt > 0 && paintAt > 0, '間引きの判定と地図の塗りが両方見つかった');
    ok(guardAt > 0 && paintAt > guardAt, '間引いた後で塗っている');
  }
}
