   ...
   熱に変わる。同じ値で呼んでも2度目からは触らないこと */
{
  ...
  // 地図。**毎フレーム塗り直さない**（塗るたびに絵を画面へ送り直すことになる）
  {
    ...
    /* 塗るのは必ず間引きの後。
       文字数の窓で見ているので、間に処理が挟まると窓を広げる必要がある
       （200→1200。間引いた後に2対2の味方の点を足す処理が入った。
       あれも毎フレーム走らせたくない側なので、ここに置くのが正しい）。
       窓を広げても、hud.minimapが間引きより前に出た場合は落ちるので意味は残る */
    ok(/_mapAcc[\s\S]{0,1200}?hud\.minimap/.test(main), '間引いた後で塗っている');
  }
}
