...
{
  /* **爪の音と溜めの予告は置いていない。**（2026-08-17に消した。
     src/core/audio.js の「爪の音と、殴る前の予告の音は置かない」を読むこと）

     ここで見張るのは「戻っていないこと」と、
     **殴られた実感(hurt)が場面の中で埋もれていないこと**の2つ。
     消した理由が「振りかぶりの音が被弾の6倍あった」なので、
     被弾の方が小さすぎないかを数字で持っておく */
  const { AudioEngine } = await import('../src/core/audio.js');
  ok(typeof AudioEngine.prototype.monsterSwipe !== 'function', '爪の音は置いていない');
  ok(typeof AudioEngine.prototype.monsterTell !== 'function', '溜めの予告の音は置いていない');
  const src = readFileSync(new URL('../src/core/audio.js', import.meta.url), 'utf8');
  ok(/爪の音と、殴る前の予告の音は置かない/.test(src), '消した理由がコードに残っている');
