...
console.log('\n[1] 品揃えと値段');
{
  ...
  /* **どの武器にも買う物がある。** 1本だけ品揃えが薄いと、
     ...
     **下限は3にしてある。** 同じ日にボーン（拳銃の4つ目）を
     「マジダサすぎる。いらない」で消して拳銃だけ3種類になったため。

     ここを4のままにすると、**気に入らない物を消すたびに検査が落ちる。**
     見た目の good/bad はこちらでは判断できない領域なので、
     消せなくなる形にするのは筋が悪い。
     下限は「買う物が無い武器を作らない」に置いて、
     4つ目を入れるかどうかは人が決める */
  const thin = perWeapon.filter((x) => x.n < 3);
  ok(thin.length === 0,
    `どの武器にも3種類以上ある${thin.length ? ` ← ${thin.map((x) => `${x.w}(${x.n})`).join('、')}` : ''}`);
