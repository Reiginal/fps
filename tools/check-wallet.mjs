...
console.log('\n[6] 台帳の作り');
{
  ...
  if (gift) {
    ...
    /* 前は「一番下に足してある」を見ていたが、それだと**後から手順を足すたびに落ちる**
       （20番のサメ返金で実際に落ちた）。本当に守りたいのは
       「一度流れた手順の前に挿し込まれて番号がずれていないこと」なので、
       流した時の番号(19)から動いていないことを見る */
    ok(gift.n === 19, '19番のまま動いていない（前に挿し込まれていない）');
  }
}
